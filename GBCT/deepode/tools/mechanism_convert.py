...
def build_arrhenius(rate, reaction_order, reaction_type):
    r"""Builds Arrhenius coefficient string based on reaction type.
    ...
    """
    if reaction_type in [ct.Reaction, ct.ElementaryReaction, ct.PlogReaction]:
        ...
        raise NotImplementedError('Reaction type not supported: ',
                                  reaction_type)

    arrhenius = [
        f'{pre_exponential_factor:.4e}', f'{rate.temperature_exponent:.3f}',
        f'{(rate.activation_energy / CALORIES_CONSTANT):.2f}'
    ]
    return '  '.join(arrhenius)


def build_falloff_arrhenius(rate, reaction_order, reaction_type,
                            pressure_limit):
    r"""Builds Arrhenius coefficient strings for falloff and chemically-activated reactions.
    ...
    """
    assert pressure_limit in ['low',
                              'high'], 'Pressure range needs to be high or low'
...
def build_falloff(parameters, falloff_function):
    r"""Creates falloff reaction Troe parameter string
    ...
    # TORE parameters: (A, T3, T1, T2[optional])
    troe_size = parameters.shape[0]
    if falloff_function == 'Troe':
        if troe_size == 4:
            falloff_string = ('TROE / ' + f'{parameters[0]}  {parameters[1]}  '
        ...
        falloff_string = (
            'SRI / ' + f'{parameters[0]}  {parameters[1]}  ' +
            f'{parameters[2]}  {parameters[3]}  {parameters[4]} /\n')
    else:
        raise NotImplementedError(
            f'Falloff function not supported: {falloff_function}')

    return falloff_string


def write_thermo_data(species_list, filename='generated_thermo.dat'):
    r"""Writes thermodynamic data to Chemkin-format file.
    ...
    with open(filename, 'w') as the_file:
        ...
        for species in species_list:
            ...
            species_string = (
                f'{species.name:<18}' + 6 * ' ' +  # date/note field
                f'{composition_string:<20}' + 'G' +  # only supports gas phase
...
def write_transport_data(species_list, filename='generated_transport.dat'):
    r"""Writes transport data to Chemkin-format file.
...
def write_chem_data(solution,
    ...
    with open(output_filename, 'w') as the_file:
        ...
        species_names = fill('  '.join(solution.species_names),
                             width=60,
        ...
        for i, reaction in enumerate(solution.reactions()):
            # print(f'{i}: {reaction}')
            # print(f'{reaction} type of reaction:{type(reaction)}')
            # print(isinstance(reaction,ct.Reaction))
            ...
            if type(reaction) in [
                    ct.Reaction, ct.ElementaryReaction, ct.ThreeBodyReaction
            ]:
                ...
                arrhenius = build_arrhenius(reaction.rates[0][1],
                                            sum(reaction.reactants.values()),
                                            ct.PlogReaction)
            else:
                raise NotImplementedError(
            ...
            if type(reaction) == ct.FalloffReaction:
                ...
                ## no need, the orginal author left a bug and I fixed it.
                # arrhenius2 = build_falloff_arrhenius(
                #     reaction.low_rate,
                #     sum(reaction.reactants.values()),
                #     ct.FalloffReaction,
                #     'high'
                #     )
                # import sys
                # print('first falloff:',arrhenius2)
                # sys.exit(0)
                # reaction_string += f'HIGH / {arrhenius} /\n'

                ## need to print additional falloff parameters if present
                # build_falloff():   Creates falloff reaction Troe parameter string
                # rate.falloff_coeffs
                # print(f'{i}: {reaction} arrhenius {arrhenius}')
                # print('reaction: ', reaction)
                # print('reaction type: ', reaction.rate.type)
                # print("reaction falloff_coeff: ", reaction.rate.falloff_coeffs)
                if reaction.rate.falloff_coeffs.size > 0:  #TROE or SRI
                    falloff_str = build_falloff(reaction.rate.falloff_coeffs,
                                                reaction.rate.type)
                    reaction_string += falloff_str

            # need to trim and print third-body efficiencies, if present
            if type(reaction) in [
                    ct.ThreeBodyReaction, ct.FalloffReaction,
                ...
                for rate in reaction.rates:
                    ...
                    arrhenius = build_arrhenius(
                        rate[1], sum(reaction.reactants.values()),
                        ct.PlogReaction)
