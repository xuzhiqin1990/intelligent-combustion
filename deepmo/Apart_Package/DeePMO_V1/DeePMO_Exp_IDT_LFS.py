...
from APART_base import _GenOneIDT, _GenOneLFS
...
class DeePMO_IDT_LFS(_DeePMO):
    """
    在 DeePMO_IDT 的基础上，增加了 LFS 的计算
    """
    def __init__(self, circ = 0, setup_file: str = './settings/setup.yaml',  SetAdjustableReactions_mode:int = None,
        ...
        if GenASampleRange:
            if self.circ == 0: 
                ...
                previous_best_chem, previous_best_chem_IDT, previous_best_chem_LFS, _ = self.SortALIST(
    ...
    def GenASampleThreshold(self, best_chem_IDT, best_chem_LFS, threshold_expand_factor = None, **kwargs):
        """
        生成采样的阈值; 根据 best_chem_IDT, best_chem_LFS 生成采样阈值. 需要在 init 中就启动
        生成的具体策略为: 
            根据 best_chem_IDT/LFS 中的每一项，计算其与真实值的误差 Rlos，乘以系数 threshold_expand_factor 后，
            以此作为 IDT/lfs 采样阈值；同时，如果 thresholds 是一个列表，将阈值设置为
            Rlos * threshold_expand_factor * thresholds[i] / thresholds[i - 1]
        add
            self.idt_threshold, self.lfs_threshold
        """
        self.GenAPARTDataLogger.info(f"Start GenASampleThreshold with best_chem_IDT: {best_chem_IDT}, best_chem_lfs: {best_chem_LFS}")
        ...
        self.GenAPARTDataLogger.info(f"Current CIRC Rlos are IDT:{idt_Rlos}, LFS:{lfs_Rlos}")
        if self.circ >= 1:
        ...
        self.GenAPARTDataLogger.info(f"Current CIRC thresholds are fixed as {np.log10(self.idt_threshold)}(np.log10) ~ {self.lfs_threshold}")
    ...
    def SortALIST(self, w1 = None, w2 = None, apart_data_path = None, experiment_time = 50, cluster_ratio = False, 
        ...
        """
        从 apart_data.npz 中筛选出来最接近真实 IDT 的采样点，以反问题权重作为 IDT  LFS 的筛选权重
        ...
        diff_lfs = np.linalg.norm(
            ...
            axis = 1, ord = ord)
        diff_idt = diff_idt[if_idt_pass]; diff_lfs = diff_lfs[if_idt_pass]
        Alist = Alist[if_idt_pass, :]; apart_data_idt = apart_data_idt[if_idt_pass, :]
        apart_data_T = apart_data_T[if_idt_pass, :]; apart_data_lfs = apart_data_lfs[if_idt_pass, :]

        diff = diff_idt + diff_lfs
        index = np.argsort(diff); Alist = Alist[index,:]
        apart_data_idt = apart_data_idt[index,:]; apart_data_T = apart_data_T[index,:];  apart_data_lfs = apart_data_lfs[index,:]
        ...
        if return_all:
            return Alist[0:experiment_time, :], apart_data_idt[0:experiment_time, :], apart_data_lfs[0:experiment_time, :], apart_data_T[0:experiment_time, :]
        elif need_idt:
    ...
    def ASample(self, sample_size = None, coreAlist = None, IDT_weight = None, LFS_weight = None,
                passing_rate_upper_limit = None, 
        ...
        if not hasattr(self, 'idt_threshold') or not hasattr(self, 'lfs_threshold'):
            idt_threshold = self.APART_args['idt_threshold']
            lfs_threshold = self.APART_args['lfs_threshold']
        ...
        if self.circ == 0 or self.circ == start_circ:
            ...
            if coreAlist is None:
                cluster_weight = kwargs.get('cluster_weight', 0.1)
                previous_coreAlist = self.SortALIST(IDT_weight, LFS_weight, 
                        apart_data_path = os.path.dirname(self.apart_data_path) + f'/apart_data_circ={self.circ - 1}.npz',
            ...
            t0 = time.time(); idt_zero_father_sample_times = 0; lfs_zero_father_sample_times = 0
                ...
                for A0 in coreAlist:
                    ...
                    if if_LFS_filter:
                        try:
                            ...
                            self.GenAPARTDataLogger.info(f"IDT+ LFS: Sampled {len(tmp_boarder_sample)} Boarder Data in this iter, total size is None! Memory usage: {psutil.Process(os.getpid()).memory_info().rss / 1024 / 1024 / 1024:.2e} GB")  
                            self.GenAPARTDataLogger.info("-·"*100)
                            if len(tmp_sample) == 0:
                                raise ValueError("tmp_sample is Empty!")
    ...
    def _TrainDataProcess(self, load_file_name = None, concat_pre = False,  rm_tree = True, 
        ...
        if extract_strategy and self.circ != 0:
            ...
            for circ in range(self.circ):
                ...
                if os.path.exists(apart_data_path) and os.path.getsize(apart_data_path) /1024 /1024 > 1:
                    Alist, IDT, LFS, T = self.SortALIST(
        ...
        all_idt_data, all_T_data, Alist_data, all_lfs_data  = \
             np.array(all_idt_data), np.array(all_T_data), np.array(Alist_data), np.array(all_lfs_data)
        ...
        assert all_idt_data.shape[0] == all_T_data.shape[0] == Alist_data.shape[0] == all_lfs_data.shape[0], \
            f"Error The shape at axis 0 is not same! all_idt_data.shape = {all_idt_data.shape}, all_T_data.shape = {all_T_data.shape}, Alist_data.shape = {Alist_data.shape}, all_lfs_data.shape = {all_lfs_data.shape}"
        # T data 做 zscore 标准化
        all_T_data, T_mean, T_std = zscore(all_T_data)
        self.APART_args.update({'T_mean': T_mean.tolist(), "T_std": T_std.tolist()})

        self.APART_args['input_dim'], self.APART_args['output_dim'] = Alist_data.shape[1], [all_idt_data.shape[1], all_lfs_data.shape[1]]
        ...
        self.TrainAnetLogger.info(f"The size of IDT &  train set is {self.APART_args['train_size']}; test set is {self.APART_args['test_size']}")
    ...
    def _DeePMO_train(self, device = None, **kwargs):
        ...
        for epoch in range(self.APART_args['epoch']):
            ...
            for _, (x_train_batch, idt_train_batch, lfs_train_batch) in enumerate(self.train_loader):  # 按照 batch 进行训练
                ...
                train_loss_batch = loss1 / loss1.detach() + loss2 / loss2.detach()
                optimizer.zero_grad()
    ...
    def SkipSolveInverse(self, w1 = None, w2 = None,  father_sample:str = None, save_dirpath = f'./inverse_skip', 
        ...
        for index in range(experiment_time):
            ...
            try:  
                ...
                Alist_data = apart_data['Alist']; idt_data = apart_data['all_idt_data']; lfs_data = apart_data['all_lfs_data'];
                # 查找 Ainit 的 index
                index = np.where(np.all(Alist_data == A_init, axis = 1))[0][0] 
                cantera_idt_data = idt_data[index]; cantera_lfs_data = lfs_data[index]
    ...
    def SortALISTStat(self, apart_data_path = None, father_sample_path = None):
        ...
        logger.info("IDT, lfs 的分布情况已保存至 ./analysis/ 中")
        return dist_A0, dist_center, std_sample


def GenOneDataIDT_LFS(IDT_condition: np.ndarray, LFS_condition: np.ndarray, Alist:list, eq_dict:dict,
                      reduced_mech:str, index:int,  my_logger:Log, tmp_chem_path:str = None, 
                      ...
                      fuel:str = None, oxidizer: str = None, IDT_fuel = None, IDT_oxidizer = None, LFS_fuel = None, LFS_oxidizer = None, **kwargs):
    ...
    idt_T = _GenOneIDT(IDT_condition, IDT_fuel, IDT_oxidizer, tmp_chem_path,  index,  my_logger, IDT_mode,
                     idt_arrays, cut_time, cut_time_alpha, idt_defined_T_diff = idt_defined_T_diff, idt_defined_time_multiple = idt_defined_time_multiple, **kwargs)
    t1 = time.time()
    ...
    if isinstance(idt_T, int) or isinstance(LFS, int):
        ...
        my_logger.info(f"Mechanism {index} : cost {t1-t0:.2f} + {time.time()-t1:.2f} s, the first IDT element is {np.log10(IDT[0]):.3e}," +
                       f"The first LFS element is {LFS[0]:.3e}, The first lfs element is {LFS[0]:.3e}" +
                       f" Memory usage: {psutil.Process(os.getpid()).memory_info().rss /1024 /1024 /1024:.2e} GB")
        np.savez(save_path, IDT = IDT, T = T, Alist = Alist, LFS = LFS)
        return None
