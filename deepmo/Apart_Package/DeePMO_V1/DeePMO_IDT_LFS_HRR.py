...
from APART_base import _GenOneIDT_HRR, _GenOneLFS
...
from .DeePMO_V1_Network import Network_PlainTripleHead, DATASET_TripleHead
from utils.cantera_multiprocess_utils import yaml2FS_Mcondition


class DeePMO_IDT_LFS_HRR(_DeePMO):
    """
    在 DeePMO_IDT_HRR 的基础上，增加了 LFS 的计算
    """
    def __init__(self, circ = 0, setup_file: str = './settings/setup.yaml', 
        ...
        if GenASampleRange:
            if self.circ == 0: 
                ...
                previous_best_chem, previous_best_chem_IDT, previous_best_chem_HRR, previous_best_chem_LFS, _ = self.SortALIST(
    ...
    def GenASampleThreshold(self, best_chem_IDT, best_chem_HRR, threshold_expand_factor = None, **kwargs):
        ...
        lfs_Rlos = np.amax(np.abs(self.reduced_lfs_data - self.true_lfs_data))
    ...
    def SortALIST(self, w1 = None, w2 = None, w3 = None, apart_data_path = None, experiment_time = 50, cluster_ratio = False, 
        ...
        """
        从 apart_data.npz 中筛选出来最接近真实 IDT 的采样点，以反问题权重作为 IDT HRR LFS 的筛选权重
        ...
        if w2 is None: w2 = self.LFS_weight
        ...
        diff_hrr = np.linalg.norm(
            ...
            axis = 1, ord = ord)
        diff_idt = diff_idt[if_idt_pass]; diff_hrr = diff_hrr[if_idt_pass]; diff_lfs = diff_lfs[if_idt_pass]
        Alist = Alist[if_idt_pass, :]; apart_data_idt = apart_data_idt[if_idt_pass, :]
        apart_data_hrr = apart_data_hrr[if_idt_pass, :]; apart_data_T = apart_data_T[if_idt_pass, :]; apart_data_lfs = apart_data_lfs[if_idt_pass, :]

        diff = diff_idt + diff_hrr + diff_lfs
        index = np.argsort(diff); Alist = Alist[index,:]
        apart_data_idt = apart_data_idt[index,:]; apart_data_T = apart_data_T[index,:]; apart_data_hrr = apart_data_hrr[index,:]; apart_data_lfs = apart_data_lfs[index,:]
        ...
        if return_all:
            return Alist[0:experiment_time, :], apart_data_idt[0:experiment_time, :], apart_data_hrr[0:experiment_time, :], apart_data_lfs[0:experiment_time, :], apart_data_T[0:experiment_time, :]
        elif need_idt:
    ...
    def ASample(self, sample_size = None, coreAlist = None, IDT_weight = None, HRR_weight = None,
                passing_rate_upper_limit = None, if_HRR_filter:bool = True, LFS_weight = None,
                IDT_reduced_threshold = None, shrink_delta = None, if_LFS_filter:bool = False,
        ...
        if not hasattr(self, 'idt_threshold') or not hasattr(self, 'hrr_threshold') or not hasattr(self, 'lfs_threshold'):
            idt_threshold = self.APART_args['idt_threshold']
            hrr_threshold = self.APART_args['hrr_threshold']
            lfs_threshold = self.APART_args['lfs_threshold']
        ...
        if self.circ == 0 or self.circ == start_circ:
            ...
            net = load_best_dnn(Network_PlainTripleHead, self.model_previous_json, device = 'cpu')
            ...
            t0 = time.time(); idt_zero_father_sample_times = 0; hrr_zero_father_sample_times = 0; lfs_zero_father_sample_times = 0
            ...
            while len(self.samples) < sample_size:
                ...
                for A0 in coreAlist:
                    ...
                    if if_HRR_filter:
                        try:
                            tmp_sample, hrr_pred_data = SampleAWithNet(net.forward_Net3, np.log10(self.true_hrr_data), threshold = np.log10(self.hrr_threshold), 
                                                        father_samples = tmp_sample, debug = True, passing_rate_upper_limit = 1,
                                                        # scalar_input = hrr_std, mean_input = hrr_mean
                                                        reduced_mech_A0 = self.reduced_mech_A0, max_uncertainty_range = self.max_uncertainty_range
                                                        )
                            if shrink_strategy:
                                tmp_boarder_sample = SampleAWithNet(net.forward_Net3, np.log10(self.true_hrr_data), threshold = np.log10(self.hrr_threshold) + 1, 
                    ...
                    if if_LFS_filter:
                        try:
                            ...
                            self.GenAPARTDataLogger.info(f"IDT + HRR + LFS: Remained {len(tmp_sample)} in this iter, pass Rate is {len(tmp_sample) / tmp_sample_size * 100} %, total size is {len(self.samples)}! cost {time.time() - t0:.2f}s; Memory usage: {psutil.Process(os.getpid()).memory_info().rss:.2e} B")  
                            self.GenAPARTDataLogger.info(f"IDT + HRR + LFS: Sampled {len(tmp_boarder_sample)} Boarder Data in this iter, total size is None! Memory usage: {psutil.Process(os.getpid()).memory_info().rss / 1024 / 1024 / 1024:.2e} GB")  
                            self.GenAPARTDataLogger.info("-·"*100)
                            if len(tmp_sample) == 0:
                                raise ValueError("tmp_sample is Empty!")
    ...
    def _TrainDataProcess(self, load_file_name = None, concat_pre = False,  rm_tree = True, 
        ...
        if extract_strategy and self.circ != 0:
            ...
            for circ in range(self.circ):
                ...
                if os.path.exists(apart_data_path) and os.path.getsize(apart_data_path) /1024 /1024 > 1:
                    Alist, IDT, HRR, LFS, T = self.SortALIST(
                    ...
                    all_hrr_data.extend(HRR); all_lfs_data.extend(LFS)
                else:
        ...
        all_idt_data, all_T_data, Alist_data, all_hrr_data, all_lfs_data  = \
             np.array(all_idt_data), np.array(all_T_data), np.array(Alist_data), np.array(all_hrr_data), np.array(all_lfs_data)
        ...
        if all_idt_data.shape[0] > expected_sample_size:
            all_idt_data, all_T_data, Alist_data, all_hrr_data, all_lfs_data = \
                ...
                all_lfs_data[:expected_sample_size]
        ...
        assert all_hrr_data.shape[0] == all_idt_data.shape[0] == all_T_data.shape[0] == Alist_data.shape[0] == all_lfs_data.shape[0], \
            f"Error The shape at axis 0 is not same! all_hrr_data.shape = {all_hrr_data.shape}, all_idt_data.shape = {all_idt_data.shape}, all_T_data.shape = {all_T_data.shape}, Alist_data.shape = {Alist_data.shape}, all_lfs_data.shape = {all_lfs_data.shape}"
        # T data 做 zscore 标准化
        all_T_data, T_mean, T_std = zscore(all_T_data)
        self.APART_args.update({'T_mean': T_mean.tolist(), "T_std": T_std.tolist()})

        self.APART_args['input_dim'], self.APART_args['output_dim'] = Alist_data.shape[1], [all_idt_data.shape[1], all_lfs_data.shape[1], all_hrr_data.shape[1]]
        ...
        dataset = DATASET_TripleHead(
            ...
            data_QoI3 = torch.tensor(all_hrr_data, dtype = torch.float32),
            device = device
        )
        ...
        for x_test, idt_test, lfs_test, hrr_test in test_loader:
            self.test_loader = (x_test.to(device), idt_test.to(device), lfs_test.to(device), hrr_test.to(device))
    ...
    def _DeePMO_train(self, device = None, HRR_train_outside_weight = 1, **kwargs):
        ...
        """
        在原有训练代码的基础上，增加了对 LFS 的训练。使用三头网络训练
        """
        ...
        ANET = Network_PlainTripleHead(input_dim, self.APART_args['hidden_units'], output_dim).to(device)
        ...
        torch.save(state, f'{model_pth_path}/Network_PlainTripleHead_initialized_circ={self.circ}.pth')  # 保存DNN模型
        ...
        for epoch in range(self.APART_args['epoch']):
            # 预测
            with torch.no_grad():
                x_dnn_test, idt_test, lfs_test, hrr_test = self.test_loader
                idt_pred, lfs_pred, hrr_pred = ANET.forward_Net1(x_dnn_test), ANET.forward_Net2(x_dnn_test), ANET.forward_Net3(x_dnn_test)
                loss1 = criterion(idt_pred, idt_test); loss3 = criterion(hrr_pred, hrr_test); loss2 = criterion(lfs_pred, lfs_test)
                test_loss = [float(loss1.cpu().detach()), float(loss2.cpu().detach()), float(loss3.cpu().detach())]
            # 训练
            idt_loss, lfs_loss, hrr_loss = 0, 0, 0
            for _, (x_train_batch, idt_train_batch, lfs_train_batch, hrr_train_batch) in enumerate(self.train_loader):  # 按照 batch 进行训练
                # x_train_batch, idt_train_batch, lfs_train_batch, hrr_train_batch = \
                #     x_train_batch.to(device), idt_train_batch.to(device), lfs_train_batch.to(device), hrr_train_batch.to(device)
                
                idt_pred, lfs_pred, hrr_pred = ANET.forward_Net1(x_train_batch), ANET.forward_Net2(x_train_batch), ANET.forward_Net3(x_train_batch)
                loss1 = criterion(idt_pred, idt_train_batch); loss3 = criterion(hrr_pred, hrr_train_batch); loss2 = criterion(lfs_pred, lfs_train_batch)

                train_loss_batch = loss1 / loss1.detach() + loss3 / loss3.detach() * HRR_train_outside_weight + loss2 / loss2.detach()
                optimizer.zero_grad()
                train_loss_batch.backward()
                optimizer.step()
                idt_loss += float(loss1.detach()); hrr_loss += float(loss3.detach()); lfs_loss += float(loss2.detach())
            
            scheduler.step()
            batch_num = len(self.train_loader)
            idt_loss /=  batch_num; hrr_loss /=  batch_num; lfs_loss /=  batch_num
            train_his1.append([idt_loss, lfs_loss, hrr_loss]); test_his1.append(test_loss)
            epoch_index.append(epoch);           

            if epoch % 5 == 0:
                GPUtil.showUtilization()
                self.TrainAnetLogger.info(f"epoch: {epoch}\t train loss: {idt_loss:.3e} + {lfs_loss:.3e} + {hrr_loss:.3e} "+
    ...
    def DeePMO_train(self, concat_pre = False, rm_tree = True, shrink_strategy = None, extract_strategy = None, 
        ...
        ax[2].semilogy(train_his1[:,2], lw=1, label='HRRtrain')
        ax[2].semilogy(test_his1[:,2], 'r', lw=1.2, label='HRRtest')
    ...
    def SkipSolveInverse(self, w1 = None, w2 = None, w3 = None, father_sample:str = None, save_dirpath = f'./inverse_skip', 
                             device = 'cpu', IDT_reduced_threshold = None, experiment_time = 15, **kwargs):
        """
        ...
        # 加载网络
        optim_net = load_best_dnn(Network_PlainTripleHead, self.model_current_json, device = device)
        for index in range(experiment_time):
            ...
            try:  
                ...
                Alist_data = apart_data['Alist']; idt_data = apart_data['all_idt_data']; hrr_data = apart_data['all_hrr_data']; lfs_data = apart_data['all_lfs_data'];
                # 查找 Ainit 的 index
                index = np.where(np.all(Alist_data == A_init, axis = 1))[0][0] 
                cantera_idt_data = idt_data[index]; cantera_hrr_data = hrr_data[index]; cantera_lfs_data = lfs_data[index]
                ...
                final_hrr = optim_net.forward_Net3(torch.tensor(A_init, dtype = torch.float32)).detach().numpy()
                # self.InverseLogger.info(f"Average Diff between final_A and A0 is {np.mean(np.abs(A_init - self.reduced_mech_A0))}; While the min and max is {np.min(np.abs(A_init - self.A0))} and {np.max(np.abs(A_init - self.A0))}")
                ...
                # log scale
                true_lfs_data = self.true_lfs_data; reduced_lfs_data = self.reduced_lfs_data; cantera_lfs_data = cantera_lfs_data
    ...
    def gather_apart_data(self, save_path = "./data/APART_data/tmp", save_file_name = None, rm_tree = True, 
        ...
        if cover or not os.path.exists(save_file_name) or os.path.getsize(save_file_name) /1024/1024 <= 1:
            all_idt_data, all_T_data, Alist_data, all_lfs_data, all_hrr_data = [], [], [], [], []
            for target_file in files:
                ...
                if os.path.getsize(target_file) >= 5:
                    ...
                    all_lfs_data.append(tmp['LFS'].tolist())
            ...
            np.savez(save_file_name,
                            ...
                            Alist = Alist_data,  all_lfs_data = all_lfs_data,
    ...
    def SortALISTStat(self, apart_data_path = None, father_sample_path = None):
        ...
        HRR = apart_data['all_hrr_data']; LFS = apart_data['all_lfs_data']
        # 样本点与原点 A0 的平均距离
        ...
        network = load_best_dnn(Network_PlainTripleHead, self.model_current_json, device = 'cpu')
        ...
        HRR_func = lambda x: network.forward_Net3(torch.tensor(x, dtype = torch.float32)).detach().numpy()
        sample_distribution_IDT(
                                np.log10(HRR),
...
def GenOneDataIDT_LFS_HRR(IDT_condition: np.ndarray, LFS_condition: np.ndarray, Alist:list, eq_dict:dict,  reduced_mech:str, 
    ...
    IDT_condition = np.array(IDT_condition); LFS_condition = np.array(LFS_condition)
    t0 = time.time()
    ...
    if isinstance(idt_T, int) or isinstance(LFS, int):
        return idt_T
    else:
        IDT, HRR, T = idt_T
        my_logger.info(f"Mechanism {index} : cost {t1-t0:.2f} + {time.time()-t1:.2f} s, the first IDT element is {np.log10(IDT[0]):.3e}," +
                       f"The first LFS element is {LFS[0]:.3e}, The first HRR element is {HRR[0]:.3e}" +
                       f"first HRR element is {HRR[0]:.3e}, Memory usage: {psutil.Process(os.getpid()).memory_info().rss /1024 /1024 /1024:.2e} GB")
        np.savez(save_path, IDT = IDT, T = T, Alist = Alist, HRR = HRR, LFS = LFS)
        return None
