...
from APART_base import _GenOneIDT, _GenOneLFS, _GenOnePSR_concentration
from APART_plot.APART_plot import compare_nn_train3, sample_distribution_IDT, compare_PSR_concentration
from utils.cantera_utils import *
from utils.setting_utils import *
from utils.yamlfiles_utils import * 
from utils.cantera_multiprocess_utils import *
from .DeePMO_V1_Network import Network_PlainTripleHead_PSRconcentration, DATASET_TripleHead


class DeePMO_IDT_LFS_concentration(_DeePMO):
    """
    在 DeePMO_IDT_LFS 的基础上增加了浓度的处理
    """
    def __init__(self, circ = 0, setup_file: str = './settings/setup.yaml',  SetAdjustableReactions_mode:int = None,
                 previous_best_chem:str = None, GenASampleRange_mode = None, **kwargs) -> None:
        ...
        if self.circ == 0: 
            ...
            previous_best_chem, previous_best_chem_IDT, previous_best_chem_LFS, previous_best_chem_PSR_concentration = self.SortALIST(
        ...
        self.GenASampleThreshold( 
            ...
            best_chem_PSR_concentration = previous_best_chem_PSR_concentration, 
            **kwargs
        )

        self.WriteCurrentAPART_args(
            IDT_weight = self.IDT_weight,
            LFS_weight = self.LFS_weight,
            PSR_concentration_weight = self.PSR_concentration_weight,
    ...
    def GenASampleRange(self, adjustable_reactions = None, l_alpha = None, r_alpha = None, mode = 'default', 
        ...
        match mode:
            ...
            case                'locally_IDT_sensitivity':  
                adjustable_reactions_eq_dict = self.eq_dict
                self.l_alpha, self.r_alpha = [], []; self.alpha_dict = {}; self.eq_dict = {}
                ...
                if not os.path.exists(IDTsensitivity_json_path):
                    IDT_sensitivity = yaml2IDT_sensitivity_Multiprocess(
                                ...
                                mode = self.IDT_mode,
                                specific_reactions = list(adjustable_reactions_eq_dict.keys()),
                ...
                # 根据 IDT_sensitivity 中的 value 对 adjustable_reactions_eq_dict 进行分组: value < 1e-4, 1e-4 < value < 1e-2, 1e-2 < value < 1e-1, 1e-1 < value < 1
                reactions_group = [
                ...
                for i, sind in enumerate(['first', 'midst', 'last']):
                    ...
                    # 更新 self.eq_dict
                    self.eq_dict.update(tmp_eq_dict)
                # self.l_alpha 按照 adjustable_reactions_eq_dict 的顺序排列，跳过不存在的反应
            ...
            case 'Scale_Sensitivity':
                ...
                alpha_dict = {k: v for k, v in alpha_dict.items() if k in original_eq_dict.keys()}
                self.alpha_dict = {
                    key: [alpha_dict[key] * -1, alpha_dict[key] * 1] for key in alpha_dict.keys()
                }
                l_alpha_dict = {key: alpha_dict[key] * -1 for key in alpha_dict.keys()}
                r_alpha_dict = {key: alpha_dict[key] * 1 for key in alpha_dict.keys()}
                self.l_alpha = eq_dict_broadcast2Alist(l_alpha_dict, self.eq_dict)
                self.r_alpha = eq_dict_broadcast2Alist(r_alpha_dict, self.eq_dict)
    ...
    def GenASampleThreshold(self, best_chem_IDT, best_chem_LFS, best_chem_PSR_concentration, 
                            threshold_expand_factor = None, **kwargs):
        """
        生成采样的阈值; 根据 best_chem_IDT, best_chem_LFS, best_chem_PSR_concentration 生成采样阈值. 需要在 init 中就启动
        生成的具体策略为: 
            根据 best_chem_IDT/LFS/PSR_concentration 中的每一项，计算其与真实值的误差 Rlos，乘以系数 threshold_expand_factor 后，
            以此作为 IDT/LFS/PSR_concentration 采样阈值；同时，如果 thresholds 是一个列表，将阈值设置为
            Rlos * threshold_expand_factor * thresholds[i] / thresholds[i - 1]
        add
            self.idt_threshold, self.lfs_threshold, self.psr_concentration_threshold
        """
        self.GenAPARTDataLogger.info(f"Start GenASampleThreshold with best_chem_IDT: {best_chem_IDT}, best_chem_lfs: {best_chem_LFS}, best_chem_PSR_concentration: {best_chem_PSR_concentration}")
        ...
        self.GenAPARTDataLogger.info(f"Current CIRC Rlos are IDT:{idt_Rlos}, LFS:{lfs_Rlos}, PSR_concentration:{psr_concentration_Rlos}")
        if self.circ >= 1:
            ...
            psr_concentration_threshold = np.array(psr_concentration_threshold)[0] if isinstance(psr_concentration_threshold, Iterable) else psr_concentration_threshold
            self.idt_threshold = min(idt_Rlos * threshold_expand_factor, idt_threshold)
            self.lfs_threshold = min(lfs_Rlos * threshold_expand_factor, lfs_threshold)
            self.psr_concentration_threshold = min(psr_concentration_Rlos * threshold_expand_factor, psr_concentration_threshold)

        self.GenAPARTDataLogger.info(f"Current CIRC thresholds are fixed as {np.log10(self.idt_threshold)}(np.log10) ~ {self.lfs_threshold} ~ {self.psr_concentration_threshold}")
    ...
    def SortALIST(self, w1 = None, w2 = None, w3 = None, apart_data_path = None, experiment_time = 50, cluster_ratio = False, 
                  ...
                  need_idt = False, need_lfs = False, return_all = False, father_sample_save_path = None, logger = None, ord = np.inf, **kwargs) -> np.ndarray:
        """
        从 apart_data.npz 中筛选出来最接近真实 IDT 的采样点，以反问题权重作为 IDT, LFS 和 PSR concentration 的筛选权重
        增加一个限制： 筛选出的结果到真实值的距离不能比 Reduced 结果差大于 IDT_reduced_threshold 倍
        1. 以此作为反问题初值; 2. 用于 ASample 筛选
        params:
            w1, w2, w3: float, 可选，IDT, LFS, PSR concentration 的权重
            ...
            return_all: 是否返回所有筛选出的 Alist
                return (Alist, idt, psr, T, lfs, psr_concentration) if True
        """
        w1 = self.IDT_weight
        w2 = self.LFS_weight
        w3 = self.PSR_concentration_weight
        ...
        apart_data_T = apart_data['all_T_data']
        ...
        # 计算 DIFF_idt, DIFF_lfs 和 DIFF_psr_concentration
        diff_idt = np.linalg.norm(
            w1 * (np.log10(apart_data_idt) - true_idt), 
            axis = 1, ord = ord)
        diff_lfs = np.linalg.norm(
            w2 * (apart_data_lfs - self.true_lfs_data), 
            axis = 1, ord = ord)
        diff_psr_concentration = np.linalg.norm(
            w3 * (apart_data_psr_concentration - self.true_psr_concentration_data), 
            axis = 1, ord = ord)
        
        diff_idt = diff_idt[if_idt_pass]; diff_lfs = diff_lfs[if_idt_pass]; diff_psr_concentration = diff_psr_concentration[if_idt_pass]
        
        Alist = Alist[if_idt_pass, :]; apart_data_idt = apart_data_idt[if_idt_pass, :]
        apart_data_lfs = apart_data_lfs[if_idt_pass, :]
        apart_data_T = apart_data_T[if_idt_pass, :]; apart_data_psr_concentration = apart_data_psr_concentration[if_idt_pass, :]

        diff = diff_idt + diff_lfs + diff_psr_concentration
        index = np.argsort(diff); Alist = Alist[index,:]
        apart_data_idt = apart_data_idt[index,:]
    ...
    def ASample(self, sample_size = None, coreAlist = None, IDT_weight = None, LFS_weight = None, PSR_concentration_weight = None,
                passing_rate_upper_limit = None, if_LFS_filter:bool = True,
        ...
        生成 ASample 的采样点; 采样点的生成方式为:
            1. 生成均匀采样点;
            2. 根据 IDT_weight, LFS_weight, PSR_concentration_weight 对均匀采样点进行加权;
            3. 根据 apart_data.npz 中的 IDT, LFS, PSR_concentration 进行筛选，筛选出最接近真实值的采样点;
        params:
        ...
        # 预设置
        self.GenAPARTDataLogger.info(f"Start The ASample Process; Here we apply IDT and LFS: {if_LFS_filter} into consideration")
        # 提取采样的左右界限 + 采样阈值
        ...
        # 检测类中是否存在 self.idt_threshold, self.lfs_threshold, self.psr_concentration_threshold
        if not hasattr(self, 'idt_threshold') or not hasattr(self, 'lfs_threshold') or not hasattr(self, 'psr_concentration_threshold'):
            idt_threshold = self.APART_args['idt_threshold']
            lfs_threshold = self.APART_args['lfs_threshold']
            psr_concentration_threshold = self.APART_args['psr_concentration_threshold']
        ...
        if self.circ == 0 or self.circ == start_circ:
            ...
            if coreAlist is None:
                cluster_weight = kwargs.get('cluster_weight', 0.1)
                previous_coreAlist = self.SortALIST(IDT_weight, LFS_weight, PSR_concentration_weight,
                        apart_data_path = os.path.dirname(self.apart_data_path) + f'/apart_data_circ={self.circ - 1}.npz',
            ...
            t0 = time.time(); idt_zero_father_sample_times = 0; lfs_zero_father_sample_times = 0; psr_concentration_zero_father_sample_times = 0
            ...
            while len(self.samples) < sample_size:
                ...
                for A0 in coreAlist:
                    ...
                    if if_LFS_filter:
                        try:
                            ...
                            self.GenAPARTDataLogger.info(f"IDT + LFS: Remained {len(tmp_sample)} in this iter, pass Rate is {len(tmp_sample) / tmp_sample_size * 100} %, total size is {len(self.samples)}! cost {time.time() - t0:.2f}s; Memory usage: {psutil.Process(os.getpid()).memory_info().rss:.2e} B")  
                    ...
                    try:
                        ...
                        self.GenAPARTDataLogger.info(f"IDT + LFS + PSR_concentration: Remained {len(tmp_sample)} in this iter, pass Rate is {len(tmp_sample) / tmp_sample_size * 100} %, total size is {len(self.samples)}! cost {time.time() - t0:.2f}s; Memory usage: {psutil.Process(os.getpid()).memory_info().rss:.2e} B")
                    ...
                    if pass_rate ** 1/3 > passing_rate_upper_limit * 0.9 and pass_rate  ** 1/3 > 0.5:
                        ...
                        self.GenAPARTDataLogger.warning(f"IDT + LFS: pass_rate is {pass_rate * 100} %, which is too high! idt_threshold have changed to log10({self.idt_threshold}) = {np.log10(self.idt_threshold)}")
                    
                    # 样本太多立即退出
                    if len(self.samples) >= sample_size * 1.2:
                        self.GenAPARTDataLogger.warning(f"IDT + LFS: Stop the Sampling Process, Total Size has come up to {len(self.samples)} data after this iter! cost {time.time() - t0:.2f}s")  
                ...
                if time.time() - t0 > sample_size * average_timeout_time:
                    ...
                    break
                self.GenAPARTDataLogger.info(f"In this Iteration, IDT + LFS: Total Size has come up to {len(self.samples)} data after this iter! cost {time.time() - t0:.2f}s")  
                self.GenAPARTDataLogger.info(f"Memory usage: {psutil.Process(os.getpid()).memory_info().rss /1024 /1024 /1024} GB")
        ...
        # 更新 idt_threshold 和 lfs_threshold
    ...
    def GenDataFuture(self, samples:np.ndarray = None, idt_cut_time_alpha = 1.5, psr_error_tol = None, 
        ...
        with ProcessPoolExecutor(max_workers = cpu_process) as exec:
            for index in range(sample_length):
                try:
                    future = exec.submit(
                            GenOneDataIDT_LFS_concentration,
    ...
    def _TrainDataProcess(self, load_file_name = None, concat_pre = False,  rm_tree = True, 
        ...
        if isinstance(load_file_name, str) and not concat_pre:
            ...
            all_idt_data, all_T_data, Alist_data, all_lfs_data, all_psr_concentration_data \
                = [], [], [], [], []
        ...
        if extract_strategy and self.circ != 0:
            ...
            for circ in range(self.circ):
                ...
                if os.path.exists(apart_data_path) and os.path.getsize(apart_data_path) /1024 /1024 > 1:
                    Alist, IDT, LFS, PSR_concentration = self.SortALIST(
        ...
        # 转化为 ndarray 加快转化 tensor 速度 + IDT 对数处理
        all_idt_data, all_T_data, Alist_data, all_lfs_data, all_psr_concentration_data = \
             np.log10(all_idt_data), np.array(all_T_data), np.array(Alist_data), np.array(all_lfs_data), np.array(all_psr_concentration_data)
        self.TrainAnetLogger.info(f'shape of all_idt_data is {all_idt_data.shape}, shape of all_T_data is {all_T_data.shape}, shape of Alist_data is {Alist_data.shape}, shape of all_lfs_data is {all_lfs_data.shape}, shape of all_psr_concentration_data is {all_psr_concentration_data.shape}')
        ...
        if all_idt_data.shape[0] > expected_sample_size:
            all_idt_data, all_T_data, Alist_data, all_lfs_data, all_psr_concentration_data = \
                all_idt_data[:expected_sample_size], all_T_data[:expected_sample_size], \
                 Alist_data[:expected_sample_size], all_lfs_data[:expected_sample_size], all_psr_concentration_data[:expected_sample_size]

        assert all_lfs_data.shape[0] == all_idt_data.shape[0] == all_T_data.shape[0] == Alist_data.shape[0], \
            f"Error The shape at axis 0 is not same! all_lfs_data.shape = {all_lfs_data.shape}, all_idt_data.shape = {all_idt_data.shape}, all_T_data.shape = {all_T_data.shape}, all_A.shape = {Alist_data.shape}"
        ...
        self.APART_args['input_dim'], self.APART_args['output_dim'] = Alist_data.shape[1], [all_idt_data.shape[1], all_lfs_data.shape[1], all_psr_concentration_data.shape[1]]
        ...
        self.TrainAnetLogger.info(f"The size of IDT & LFS & Concentration train set is {self.APART_args['train_size']}; test set is {self.APART_args['test_size']}")
        ...
        dataset = DATASET_TripleHead(
            ...
            data_QoI2 = torch.tensor(all_lfs_data, dtype = torch.float32),
            data_QoI3= torch.tensor(all_psr_concentration_data, dtype = torch.float32),
            device = device
        )
    ...
    def _DeePMO_train(self, device = None, LFS_train_outside_weight = 1, **kwargs):
        ...
        for epoch in range(epoch_nums):
            ...
            if (epoch == 0) or ((epoch - 25) % 50 == 0):
                mkdirplus(f'{model_pth_path}/tmp') # 创建临时保存网络参数的文件夹
    ...
    def DeePMO_train(self, concat_pre = False, rm_tree = True, shrink_strategy = None, extract_strategy = None, 
        ...
        best_ppth = self._DeePMO_train(device = device, PSR_train_outside_weight = PSR_train_outside_weight)
        ...
        ax[2].semilogy(train_his1[:,2], lw=1, label='Concentrationtrain')
        ax[2].semilogy(test_his1[:,2], 'r', lw=1.2, label='Concentrationtest')
    ...
    def SkipSolveInverse(self, father_sample:str = None, save_dirpath = f'./inverse_skip', device = 'cpu', IDT_reduced_threshold = None, experiment_time = 15, **kwargs):
        """
        ...
        for index in range(experiment_time):
            ...
            try:  
                ...
                lfs_data = apart_data['all_lfs_data']; psr_concentration_data = apart_data['all_psr_concentration_data']
                # 查找 Ainit 的 index
                init_index = np.where(np.all(Alist_data == A_init, axis = 1))[0][0] 
                cantera_idt_data = idt_data[init_index]; cantera_lfs_data = lfs_data[init_index]
...
def GenOneDataIDT_LFS_concentration(IDT_condition: np.ndarray, LFS_condition: np.ndarray, PSR_concentration_kwargs: dict,Alist:list, eq_dict:dict, reduced_mech:str, index:int, my_logger:Log, tmp_chem_path:str = None, fuel:str = None, oxidizer: str = None, 
                                    IDT_mode = 0, remove_chem = True, idt_arrays:np.ndarray = 1, cut_time:np.ndarray = None, cut_time_alpha = 10, save_path = 'data/APART_data/tmp',
                                    IDT_fuel:str = None, IDT_oxidizer: str = None,  LFS_fuel = None, LFS_oxidizer = None, **kwargs):
    """
    将 IDT 和 PSR 结合起来
    ...
    if isinstance(idt_T, int) or isinstance(LFS, int) or isinstance(PSR_concentration, int):
        return index
    else:
        IDT, T = idt_T
        my_logger.info(f"Mechanism {index} : cost {t2 - t1:.2f} + {t1 - t0:.2f} + {t3 - t2:.2f}  s, the first IDT element is {np.log10(IDT[0]):.3e}, " + 
                       f"first LFS element is {LFS[0]:.3e}, the first PSR concentration is {PSR_concentration[0:1]} Memory usage: {psutil.Process(os.getpid()).memory_info().rss /1024 /1024 /1024:.2e} GB")
        np.savez(save_path, IDT = IDT, T = T, Alist = Alist,  LFS = LFS, PSR_concentration = PSR_concentration)
        return None
