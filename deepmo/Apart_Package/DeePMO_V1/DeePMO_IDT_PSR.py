...
from APART_base import _GenOneIDT, _GenOnePSR, _GenOnePSR_plus_PSRex
from APART_plot.APART_plot import compare_nn_train3, sample_distribution_IDT, sample_distribution_PSR, sample_distribution_PSRex
# from APART_base import GenOneDataIDTPSR
from utils.cantera_utils import *
from utils.cantera_PSR_definations import *
from utils.setting_utils import *
from utils.yamlfiles_utils import * 
from APART_plot.Result_plot import CompareDRON_PSR_lineplot
from .DeePMO_V1_Network import *
from .DeePMO_IDT_PSRextinction import GenOneDataIDTPSRextinction

class DeePMO_IDT_PSR(_DeePMO):
    """
    DeePMO_IDT_PSR: DeePMO with IDT and PSR
    基本结构沿用了 DeePMO 的结构，但是在 DeePMO 的基础上增加了 IDT 和 PSR 的数据集; 与 DeePMO_IDT_PSRex 的区别在于虽然保留了 PSRex 的基本结构
    但是一般而言并不打算启用 PSRex 的数据集，专注于 PSR 网络的学习和优化；针对一般情况，可以在 DeePMO_IDT_PSRex 中开始优化 PSRex 后，再切换到
    DeePMO_IDT_PSR 中进行优化 PSR 网络。在优化过程中，保持 PSRex 指标不变差即可
    20230810：启用 Network_PSR_PlainMLP 作为 PSR 网络的结构
    """
    def __init__(self, circ = 0, basic_set = True, GenASampleRange = True, setup_file: str = './settings/setup.yaml', 
                 ...
                 PSRex_cutdown = True, **kwargs) -> None:
        ...
        self.psr_error_tol = self.APART_args.get('psr_error_tol', 100)
        ...
        self.data_normalization_method = self.APART_args.get('data_normalization_method', 'None')
        ...
        if GenASampleRange:
            if previous_best_chem is None:
                if self.circ == 0: 
                    ...
                    previous_best_chem_PSR = self.reduced_psr_data
                    ...
                    previous_best_chem, previous_best_chem_IDT, previous_best_chem_PSR, previous_best_chem_PSRex, _ = self.SortALIST(
            ...
            self.GenASampleThreshold( 
                best_chem_IDT = previous_best_chem_IDT, best_chem_PSR = previous_best_chem_PSR, best_chem_PSRex = previous_best_chem_PSRex,
                **kwargs
            )
        else:
    ...
    def cal_reduced_PSRex(self, need_true = False, save_path = "./data/true_data/true_psrex.npz"):
        ...
        if save_path is not None and os.path.exists(save_path):
            ...
            self.GenAPARTDataLogger.info(f"True extinction time loaded from {save_path}")
        else:
            if need_true:
                t0 = time.time()
                self.GenAPARTDataLogger.info(f"Calculating true extinction time...")
                self.true_extinction_time = yaml2psr_extinction_time(
                    ...
                    psr_tol = self.psr_error_tol, init_res_time = self.init_res_time, exp_factor = self.PSRex_decay_rate,
                )
    ...
    def GenASampleThreshold(self, best_chem_IDT, best_chem_PSR, best_chem_PSRex, threshold_expand_factor = None, **kwargs):
        """
        生成采样的阈值; 根据 best_chem_IDT, best_chem_PSR, best_chem_PSRex 生成采样阈值. 需要在 init 中就启动
        生成的具体策略为: 
            根据 best_chem_IDT/PSR/PSRex 中的每一项，计算其与真实值的误差 Rlos，乘以系数 threshold_expand_factor 后，
            以此作为 IDT/PSR/PSRex 采样阈值；同时，如果 thresholds 是一个列表，将阈值设置为
            Rlos * threshold_expand_factor * thresholds[i] / thresholds[i - 1]
        add
            self.idt_threshold, self.psr_threshold, self.psr_extinction_threshold
        """
        self.GenAPARTDataLogger.info(f"Start GenASampleThreshold with best_chem_IDT: {best_chem_IDT}, best_chem_PSR: {best_chem_PSR}, best_chem_PSRex: {best_chem_PSRex}")
        idt_threshold = self.APART_args['idt_threshold']
        psr_threshold = self.APART_args['psr_threshold']
        ...
        psr_threshold = np.array(psr_threshold)[self.circ - 1] if isinstance(psr_threshold, Iterable) else psr_threshold
        ...
        psr_Rlos = np.amax(np.abs(best_chem_PSR - self.true_psr_data))
        psrex_Rlos = 2 ** np.amax(np.abs(np.log2(best_chem_PSRex) - np.log2(self.true_extinction_time)))
        self.GenAPARTDataLogger.info(f"Current CIRC Rlos are {idt_Rlos} ~ {psr_Rlos} ~ {psrex_Rlos}")
        if self.circ >= 1:
            ...
            if isinstance(psr_threshold, Iterable):
                self.psr_threshold = min(psr_Rlos * threshold_expand_factor * psr_threshold[self.circ] / psr_threshold[self.circ - 1], psr_threshold)
            else:
                self.psr_threshold = min(psr_Rlos * threshold_expand_factor, psr_threshold)
            ...
            if isinstance(psrex_threshold, Iterable):
                ...
                self.psrex_threshold = min(psrex_Rlos * threshold_expand_factor, psrex_threshold)
        else:
            idt_threshold = np.array(idt_threshold)[0] if isinstance(idt_threshold, Iterable) else idt_threshold
            psr_threshold = np.array(psr_threshold)[0] if isinstance(psr_threshold, Iterable) else psr_threshold
            psrex_threshold = np.array(psrex_threshold)[0] if isinstance(psrex_threshold, Iterable) else psrex_threshold 
            self.idt_threshold = min(idt_Rlos * threshold_expand_factor, idt_threshold)
            self.psr_threshold = min(psr_Rlos * threshold_expand_factor, psr_threshold)
            self.psrex_threshold = min(psrex_Rlos * threshold_expand_factor, psrex_threshold)

        self.GenAPARTDataLogger.info(f"Current CIRC thresholds are fixed as {self.idt_threshold} ~ {self.psr_threshold} ~ {self.psrex_threshold}")
        self.GenAPARTDataLogger.info("=" * 50)
        # self.WriteCurrentAPART_args(idt_threshold = self.idt_threshold, psr_threshold = self.psr_threshold, psrex_threshold = self.psrex_threshold)


    def SortALIST(self, w1 = None, w2 = None, apart_data_path = None, experiment_time = 50, cluster_ratio = False, 
                  IDT_reduced_threshold = None, PSR_reduced_threshold = None, 
                  SortALIST_T_threshold:float = None, SortALIST_T_threshold_ratio:list|float = None,
                  need_idt = False, need_psr = False, return_all = False, father_sample_save_path = None,
                  PSR_ignition_Success_Threshold: float = False, logger = None, ord = np.inf, PSRex_cutdown = None, **kwargs):
        """
        ...
        params:
            ...
            PSR_ignition_Success_Threshold: 筛选出的结果中，PSR 点火成功的比例不能低于 PSR_ignition_Success_Threshold
            need_idt: 如果需要筛选出的 Alist 对应的 apart_data， 将此键值改为 True
        """
        ...
        apart_data_psrex = apart_data['all_psr_extinction_data']
        true_psr = self.true_psr_data; reduced_psr = self.reduced_psr_data
        ...
        if not PSR_ignition_Success_Threshold is False:
            if_idt_pass *= np.all(np.abs(apart_data_psr - self.true_psr_data) <= PSR_ignition_Success_Threshold, axis = 1)

        # 筛选出的结果不能比 Reduced 结果差大于  PSR_reduced_threshold 倍
        if not PSR_reduced_threshold is None:
            if_idt_pass *= np.all(np.abs(true_psr - apart_data_psr) <= PSR_reduced_threshold * np.abs(reduced_psr - true_psr), axis = 1)
            if if_idt_pass.sum() == 0: logger.warning(
                f"No sample pass the PSR_reduced_threshold filter! apart_data - true = {np.mean(np.abs(true_psr - apart_data_psr))} and true - reduce = {np.abs(reduced_psr - true_psr)}"
                )
        ...
        diff_idt = np.linalg.norm(
            ...
            axis = 1, ord = ord)
        diff_psr = np.linalg.norm(
            w2 / 100 * (true_psr - apart_data_psr), 
            axis = 1, ord = 2)
        diff_psrex = np.linalg.norm(
            apart_data_psrex_log2 - np.log2(self.true_extinction_time), 
            axis = 1, ord = ord)
        
        diff_idt = diff_idt[if_idt_pass]; Alist = Alist[if_idt_pass, :]; apart_data_idt = apart_data_idt[if_idt_pass, :]
        diff_psr = diff_psr[if_idt_pass]; apart_data_psr = apart_data_psr[if_idt_pass, :]; apart_data_psrex = apart_data_psrex[if_idt_pass, :]
        diff_psrex = diff_psrex[if_idt_pass]
        # DIFF_psr 暂时删除或者只有 diff_psrex 非常小才启用
        if diff_psrex.max() < 1e-3:
            diff = diff_idt + diff_psrex + diff_psr
        diff = diff_idt + diff_psrex
        ...
        apart_data_T = apart_data_T[index,:]
        ...
        if return_all:
            return Alist[0:experiment_time, :], apart_data_idt[0:experiment_time, :] , apart_data_psr[0:experiment_time, :], apart_data_psrex[0:experiment_time, :], apart_data_T[0:experiment_time, :]
        elif need_idt and need_psr:
            return Alist[0:experiment_time, :], apart_data_idt[0:experiment_time, :] , apart_data_psr[0:experiment_time, :]          
        elif need_idt and not need_psr:
            return Alist[0:experiment_time, :], apart_data_idt[0:experiment_time, :]
        elif need_psr and not need_idt:
            return Alist[0:experiment_time, :], apart_data_psr[0:experiment_time, :]
        else:
            return Alist[0:experiment_time, :]
        

    def ASample(self, sample_size = None, coreAlist = None, SortALIST_w1 = None, SortALIST_w2 = None,
                passing_rate_upper_limit = None, if_PSR_filter:bool = True, if_PSRex_filter:bool = True,
        ...
        """
        重写了 APART.ASample_IDT_PSR 函数，主要是为了增加针对不同反应调节采样范围的函数
        202230730: 增加了通过率上限 passing_rate_upper_limit, 是否启用 PSR 和 PSRex 训练的 if_PSR_filter 与 if_PSRex_filter
        """
        assert self.PSR_mode == True
        np.set_printoptions(precision = 2, suppress = True); t0 = time.time()

        # 预设置
        self.GenAPARTDataLogger.info(f"Start The ASample Process; Here we apply three aspect into consideration: IDT: True, PSR: {if_PSR_filter} and PSRextinction: {if_PSRex_filter}")
        # 提取采样的左右界限 + 采样阈值
        if SortALIST_w1 is None: SortALIST_w1 = self.IDT_weight
        if SortALIST_w2 is None: SortALIST_w2 = self.PSR_weight

        # 检测类中是否存在 self.idt_threshold, self.psr_threshold, self.psrex_threshold
        if not hasattr(self, 'idt_threshold') or not hasattr(self, 'psr_threshold') or not hasattr(self, 'psrex_threshold'):
            idt_threshold = self.APART_args['idt_threshold']
            psr_threshold = self.APART_args['psr_threshold']
            psrex_threshold = self.APART_args['psrex_threshold']
            self.idt_threshold = np.array(idt_threshold)[self.circ - 1] if isinstance(idt_threshold, Iterable) else idt_threshold
            self.psr_threshold = np.array(psr_threshold)[self.circ - 1] if isinstance(psr_threshold, Iterable) else psr_threshold
        ...
        if self.circ == 0 or self.circ == start_circ:
            ...
            IDT_net = load_best_dnn(Network_PlainSingleHead, self.model_previous_json, device = 'cpu', prefix = "IDT")
            PSR_net = load_best_dnn(Network_PSR_PlainMLP, self.model_previous_json, device = 'cpu', DNN_instantiation_keys = (
                "PSR_best_ppth", "PSR_layer_nums", "PSR_input_dim", "hidden_units", "PSR_output_dim", 
            ))
            PSRex_net = load_best_dnn(Network_PlainSingleHead, self.model_previous_json, device = 'cpu', prefix = "PSR_extinction")
            # 将 true psr data 和 psr_threshold 标准化
            psr_mean = np.array(json_data['psr_mean']); psr_std = np.array(json_data['psr_std'])
            PSR_reverse_DataNormalization = partial(ReverseDataNormalizationProcess, method = self.data_normalization_method,
                                                    mean = psr_mean, std = psr_std, 
                                                    replace_maskon_items = np.repeat(
                                                            self.PSR_condition,
                                                            repeats = 3,
                                                            axis = 0
                                                    )[:,1])

            self.GenAPARTDataLogger.info(f"idt_threshold: log10({self.idt_threshold}) = {np.log10(self.idt_threshold)}; psr_threshold: {np.amin(self.psr_threshold)} ~ {np.amax(self.psr_threshold)}; sample_size: {sample_size}; father sample size: {core_size}")
            ...
            if coreAlist is None:
                cluster_weight = kwargs.get('cluster_weight', 0.1)
                previous_coreAlist = self.SortALIST(SortALIST_w1, SortALIST_w2, 
                        apart_data_path = os.path.dirname(self.apart_data_path) + f'/apart_data_circ={self.circ - 1}.npz',
                        experiment_time = core_size, psr_mean = psr_mean, psr_std = psr_std, IDT_reduced_threshold = IDT_reduced_threshold, 
            ...
            t0 = time.time(); idt_zero_father_sample_times = 0; psr_zero_father_sample_times = 0; psrex_zero_father_sample_times = 0
            ...
            while len(self.samples) < sample_size:
                ...
                for A0 in coreAlist:
                    try:  
                        tmp_sample, idt_pred_data = SampleAWithNet(IDT_net, np.log10(self.true_idt_data), threshold = np.log10(self.idt_threshold), 
                        ...
                        if shrink_strategy:
                            tmp_boarder_sample = SampleAWithNet(IDT_net, np.log10(self.true_idt_data), threshold = np.log10(self.idt_threshold) + 1, 
                    ...
                    tmp_sample, psr_pred_data = SampleAWithNet(PSR_net.forward_squeeze, self.true_psr_data, threshold = self.psr_threshold, 
                                                father_samples = tmp_sample, debug = True, reverse_DataNormalization = PSR_reverse_DataNormalization,)
                    if shrink_strategy:
                        tmp_boarder_sample = SampleAWithNet(PSR_net.forward_squeeze, self.true_psr_data, threshold = self.psr_threshold + 1, 
                                                father_samples = tmp_boarder_sample, debug = False, reverse_DataNormalization = PSR_reverse_DataNormalization,)                      
                    psr_Rlos = np.abs(np.mean(psr_pred_data - self.true_psr_data, axis = 0))
                    self.GenAPARTDataLogger.info(f"PSR: On Average, Working Condition Index which not satisfy threshold IS {np.where(psr_Rlos > self.true_psr_data,)[0]}")
                    # self.GenAPARTDataLogger.info(f"PSR: On Average, Working Condition which not satisfy threshold IS {np.unique(np.tile(self.PSR_condition, (1,3))[np.where(psr_Rlos > true_psr)[0],:], axis = 0)}")
                    self.GenAPARTDataLogger.info(f"true PSR is {self.true_psr_data,}")
                    self.GenAPARTDataLogger.info(f"First element of sample prediction PSR is {psr_pred_data[0]}")
                    self.GenAPARTDataLogger.info(f"Abs Difference between true PSR and sample prediction is {psr_Rlos}")
                    self.GenAPARTDataLogger.info(f"IDT + PSR: Remained {len(tmp_sample)} in this iter, pass Rate is {len(tmp_sample) / tmp_sample_size * 100} %, total size is {len(self.samples)}! cost {time.time() - t0:.2f}s; Memory usage: {psutil.Process(os.getpid()).memory_info().rss:.2e} B")  
                    self.GenAPARTDataLogger.info(f"IDT + PSR: Sampled {len(tmp_boarder_sample)} Boarder Data in this iter, total size is None! Memory usage: {psutil.Process(os.getpid()).memory_info().rss / 1024 / 1024 / 1024:.2e} GB")  
                    self.GenAPARTDataLogger.info("-·"*100)
                    # if len(tmp_sample) == 0:
                    #     raise ValueError("tmp_sample is Empty!")

                    if len(tmp_sample) == 0:
                        self.GenAPARTDataLogger.warning(f"PSR: tmp_sample is Empty in This circle! psr_zero_father_sample_times = {psr_zero_father_sample_times}")
                        self.GenAPARTDataLogger.info("-·"*100)
                        psr_zero_father_sample_times += 1
                        continue
                    try:
                        tmp_sample, psrex_pred_data = SampleAWithNet(PSRex_net, np.log2(self.true_extinction_time), threshold = np.log2(self.psrex_threshold), 
                                                    father_samples = tmp_sample, loss_func = psrex_loss_func, passing_rate_upper_limit = np.sqrt(passing_rate_upper_limit), debug = True)
                        if shrink_strategy:
                        ...
                        self.GenAPARTDataLogger.info(f"IDT + PSR + PSRex: Sampled {len(tmp_sample)} in this iter, pass Rate is {len(tmp_sample) / tmp_sample_size * 100} %, total size is {len(self.samples) + len(tmp_sample)}! cost {time.time() - t0:.2f}s; Memory usage: {psutil.Process(os.getpid()).memory_info().rss:.2e} B")  
                        self.GenAPARTDataLogger.info(f"IDT + PSR + PSRex: Sampled {len(tmp_boarder_sample)} Boarder Data in this iter, total size is None! Memory usage: {psutil.Process(os.getpid()).memory_info().rss /1024 /1024 /1024:.2e} GB")  
                        self.GenAPARTDataLogger.info("——"*100)
                        if len(tmp_sample) == 0:
                            raise ValueError("tmp_sample is Empty!")
                    except ValueError or IndexError:
                    ...
                    if pass_rate ** 1/2 > passing_rate_upper_limit * 0.9 and pass_rate  ** 1/2 > 0.5:
                        ...
                        self.GenAPARTDataLogger.warning(f"IDT + PSR + PSRex: pass_rate is {pass_rate * 100} %, which is too high! idt_threshold have changed to log10({self.idt_threshold}) = {np.log10(self.idt_threshold)}; psrex_threshold have changed to log2({self.psrex_threshold}) = {np.log2(self.psrex_threshold)}")
                    
                    # 样本太多立即退出
                    if len(self.samples) >= sample_size * 1.2:
                        self.GenAPARTDataLogger.warning(f"IDT + PSR + PSRex: Stop the Sampling Process, Total Size has come up to {len(self.samples)} data after this iter! cost {time.time() - t0:.2f}s")  
                ...
                if time.time() - t0 > sample_size * average_timeout_time:
                    ...
                    break
                self.GenAPARTDataLogger.info(f"In this Iteration, IDT + PSR + PSRex: Total Size has come up to {len(self.samples)} data after this iter! cost {time.time() - t0:.2f}s")  
                self.GenAPARTDataLogger.info(f"Memory usage: {psutil.Process(os.getpid()).memory_info().rss /1024 /1024 /1024} GB")
                ...
                if psr_zero_father_sample_times > 10* len(coreAlist):
                    self.psr_threshold = self.psr_threshold * 1.1
                    self.GenAPARTDataLogger.warning(f"PSR: psr_threshold: {np.amin(self.psr_threshold)} ~ {np.amax(self.psr_threshold)};")
                    psr_zero_father_sample_times = 0 
        ...
        # 更新 idt_threshold 和 psr_threshold 和 psrex_threshold
        self.APART_args['idt_threshold'] = self.idt_threshold
        self.APART_args['psr_threshold'] = self.psr_threshold
        ...
        self.GenAPARTDataLogger.info(f"End The ASample_IDT_PSR Progress! The size of samples is {len(self.samples)}, the size of boarder_samples is None, cost {time.time() - t0:.2f}s")  
        return self.samples
    ...
    def _TrainDataProcess(self, load_file_name = None,
                              ...
                              net_data_dirpath = "./data/APART_data/ANET_data",
                              PSR_ignition_Success_Threshold = 250,  **kwargs) -> None:
        """
        基本上和 APART 中的对应函数功能相同，增加了将 0D 末态温度同时也作为训练网络数据集的一部分的功能
        20230825: 增加了 PSR_ignition_Success_Threshold 参数，用于过滤掉 PSR 误差大于该值的数据点
        """
        # gather_data 部分
        ...
        if isinstance(load_file_name, str) and not concat_pre:
            ...
            all_idt_data, all_T_data, Alist_data, all_psr_data = [], [], [], []
        ...
        if extract_strategy and self.circ != 0:
            ...
            for circ in range(self.circ):
                ...
                if os.path.exists(apart_data_path) and os.path.getsize(apart_data_path) /1024 /1024 > 1:
                    Alist, IDT, PSR, PSRex, T = self.SortALIST(
        ...
        all_idt_data, all_T_data, all_psr_data, Alist_data, all_psr_extinction_data  = \
             np.log10(all_idt_data), np.array(all_T_data), np.array(all_psr_data), np.array(Alist_data), np.log2(all_psr_extinction_data)
        ...
        self.TrainAnetLogger.info(f"The size of all_idt_data is {all_idt_data.shape}, all_T_data is {all_T_data.shape}, \n all_psr_data is {all_psr_data.shape}, Alist_data is {Alist_data.shape}, all_psr_extinction_data is {all_psr_extinction_data.shape}")
        ...
        # 删除 PSR 温度表现很差的样本点
        # psr_init_tem = np.repeat(self.PSR_condition[:,1], self.RES_TIME_LIST.shape[1])
        # if_pass = np.all(all_psr_extinction_data - np.log2(self.true_extinction_time) <= 0, axis = 1)
        if_pass = np.all(np.abs(all_psr_extinction_data - np.log2(self.true_extinction_time)) <= 0, axis = 1) *\
            np.all(np.abs(all_psr_data - self.true_psr_data) <= PSR_ignition_Success_Threshold, axis = 1)
        self.TrainAnetLogger.info(f"DataProcessing: PSR: if_pass rate is {np.sum(if_pass) / if_pass.shape[0] * 100} %")
        passed_idt_data, passed_T_data, passed_psr_data, passed_A = \
            all_idt_data[if_pass], all_T_data[if_pass], all_psr_data[if_pass], Alist_data[if_pass]
        ...
        if passed_idt_data.shape[0] > expected_sample_size:
            passed_idt_data, passed_T_data, passed_psr_data, passed_A = \
                ...
                passed_psr_data[:expected_sample_size], passed_A[:expected_sample_size]
        ...
        assert passed_idt_data.shape[0] == passed_T_data.shape[0] == passed_psr_data.shape[0] == passed_A.shape[0], \
            f"Error! passed_idt_data.shape = {passed_idt_data.shape}, passed_T_data.shape = {passed_T_data.shape}, passed_psr_data.shape = {passed_psr_data.shape}, passed_A.shape = {passed_A.shape}; which not same at axis 0"
        # PSR data, T data 做 DataNormalizationProcess 标准化
        passed_psr_data, psr_mean, psr_std = DataNormalizationProcess(passed_psr_data, method = self.data_normalization_method, mask_on_threshold = np.amax(self.PSR_condition[:,1]))
        passed_T_data, T_mean, T_std = DataNormalizationProcess(passed_T_data, method = self.data_normalization_method,)
        self.APART_args.update({'psr_mean': psr_mean, "psr_std": psr_std})
        self.APART_args.update({'T_mean': T_mean, "T_std": T_std})

        self.APART_args['IDT_input_dim'], self.APART_args['IDT_output_dim'] = passed_A.shape[1], passed_idt_data.shape[1]
        self.APART_args['IDT_train_size']  = int(passed_A.shape[0] * rate)
        self.APART_args['IDT_test_size'] = passed_A.shape[0] - self.APART_args['IDT_train_size']

        # 将 PSR reshape 掉
        passed_psr_data = passed_psr_data.reshape(passed_A.shape[0], len(self.PSR_condition), -1)
        self.APART_args['PSR_input_dim'], self.APART_args['PSR_output_dim'] = passed_A.shape[1], passed_psr_data.shape[1]
        self.APART_args['PSR_layer_nums'] = passed_psr_data.shape[2]
        print(f"passed_psr_data.shape = {passed_psr_data.shape}; input dim = {self.APART_args['PSR_input_dim']}, output dim = {self.APART_args['PSR_output_dim']}, layer nums = {self.APART_args['PSR_layer_nums']}")
        self.APART_args['PSR_train_size'] = int(passed_A.shape[0] * rate)
        self.APART_args['PSR_test_size'] = passed_A.shape[0] - self.APART_args['PSR_train_size']
        ...
        self.TrainAnetLogger.info(f"The size of IDT & PSR train set is {self.APART_args['IDT_train_size'], self.APART_args['PSR_train_size']}; test set is {self.APART_args['IDT_test_size'], self.APART_args['PSR_test_size']}")
        ...
        IDT_dataset = DATASET_SingleHead(
            data_A = torch.tensor(passed_A, dtype = torch.float32),
            data_idt = torch.tensor(passed_idt_data, dtype = torch.float32),
            device = device,
        )
        IDT_train_data, IDT_test_data = random_split(IDT_dataset
            , lengths = [self.APART_args['IDT_train_size'], self.APART_args['IDT_test_size']])

        self.IDT_train_loader = DataLoader(IDT_train_data, 
                                       shuffle = True, 
                                       batch_size = self.APART_args['batch_size'],
                                       drop_last = True,)
        
        # 单独写出来 test 的数据
        IDT_test_loader = DataLoader(IDT_test_data, 
                                shuffle = False, 
                                batch_size = len(IDT_test_data),)

        for x_test, idt_test in IDT_test_loader:
            self.IDT_test_loader = (x_test.to(device), idt_test.to(device))
 
        # PSR 数据集单独列出
        PSR_dataset = DATASET_SingleHead(
            data_A = torch.tensor(passed_A, dtype = torch.float32),
            data_QoI = torch.tensor(passed_psr_data, dtype = torch.float32),
            device = device,
        )
        PSR_train_data, PSR_test_data = random_split(PSR_dataset
            , lengths = [self.APART_args['PSR_train_size'], self.APART_args['PSR_test_size']])
        self.PSR_train_loader = DataLoader(PSR_train_data,
                                                        shuffle = True,
                                                        batch_size = self.APART_args['batch_size'],)   
        # 单独写出来 test 的数据
        PSR_test_loader = DataLoader(PSR_test_data,
                                     shuffle = False,
                                     batch_size = len(PSR_test_data))
        for x_test, psr_test in PSR_test_loader:
            self.PSR_test_loader = (x_test.to(device), psr_test.to(device))
        ...
        if not net_data_dirpath is None:
            np.savez(
                net_data_dirpath + f"/ANET_Dataset_IDTtrain={self.APART_args['IDT_train_size']}_circ={self.circ}.npz",
                IDT_train_data_x = IDT_dataset[IDT_train_data.indices][0].cpu().numpy(),
                IDT_train_data_y = IDT_dataset[IDT_train_data.indices][1].cpu().numpy(),
                IDT_test_data_x =  IDT_dataset[IDT_test_data.indices][0].cpu().numpy(),
                IDT_test_data_y =  IDT_dataset[IDT_test_data.indices][1].cpu().numpy(),
                PSR_train_data_x = PSR_dataset[PSR_train_data.indices][0].cpu().numpy(),
                PSR_train_data_y = PSR_dataset[PSR_train_data.indices][1].cpu().numpy(),
                PSR_test_data_x = PSR_dataset[PSR_test_data.indices][0].cpu().numpy(),
                PSR_test_data_y = PSR_dataset[PSR_test_data.indices][1].cpu().numpy(),
    ...
    def _DeePMO_train(self, device = None, **kwargs):
        ...
        """
        在原有训练代码的基础上，增加了对 PSR_extinction 的训练。首先进行 IDT / PSR 的训练，
        紧接着再次单独训练一个 PSR extinction 网络
        """
        ...
        t0 = time.time()
        input_dim, output_dim = self.APART_args['IDT_input_dim'], self.APART_args['IDT_output_dim']
        tmp_save_path = mkdirplus(f'{model_pth_path}/tmp')
        self.TrainAnetLogger.info(f'A -> IDT training has started...; circ = {self.circ}')
        Network_PlainSingleHead = Network_PlainSingleHead(input_dim, self.APART_args['hidden_units'], output_dim).to(device)
        if self.APART_args['optimizer'] == 'Adam': optimizer = optim.Adam(Network_PlainSingleHead.parameters(), lr = self.APART_args['learning_rate'])
        if self.APART_args['optimizer'] == 'SGD':  optimizer = optim.SGD(Network_PlainSingleHead.parameters(), lr = self.APART_args['learning_rate'], momentum = 0.1)
        # 第一次保存初始模型
        state = {'model':Network_PlainSingleHead.state_dict()}   
        ...
        for epoch in range(self.APART_args['epoch']):
            # 预测
            with torch.no_grad():
                x_dnn_test, idt_test = self.IDT_test_loader
                idt_pred = Network_PlainSingleHead.forward(x_dnn_test)
                loss1 = criterion(idt_pred, idt_test)
                test_loss = float(loss1.cpu().detach())
            # 训练
            idt_loss = 0
            for x_train_batch, idt_train_batch in self.IDT_train_loader:  # 按照 batch 进行训练
                # x_train_batch, idt_train_batch = x_train_batch.to(device), idt_train_batch.to(device)
                idt_pred = Network_PlainSingleHead.forward(x_train_batch)
                train_loss_batch = criterion(idt_pred, idt_train_batch)
                optimizer.zero_grad()
                train_loss_batch.backward()
                optimizer.step()
                idt_loss += float(train_loss_batch.detach())
            
            scheduler.step()
            batch_num = len(self.IDT_train_loader)
            idt_loss /=  batch_num
            train_his1.append(idt_loss); test_his1.append(test_loss); epoch_index.append(epoch);           

            if epoch % 5 == 0:
                GPUtil.showUtilization()
                self.TrainAnetLogger.info(f"epoch: {epoch}\t train loss: {idt_loss:.3e} "+ f"test loss: {test_loss:.3e}"+
                                            f"time cost: {int(time.time()-t0)} s   lr:{optimizer.param_groups[0]['lr']:.2e}" 
                                            # 打印显存和最大显存
                                            + f"memory used: {torch.cuda.memory_allocated(device) / 1024 / 1024:.2f} MB, max memory used: {torch.cuda.max_memory_allocated(device) / 1024 / 1024:.2f} MB"
                                            # 打印 GPU 使用率
                                            + f"GPU memory used rate: {torch.cuda.memory_allocated(device) / torch.cuda.max_memory_allocated(device) * 100:.2f} %"
                                            )
            if (epoch == 0) or ((epoch - 25) % 50 == 0):
                state = {'model':Network_PlainSingleHead.state_dict(), 'optimizer':optimizer.state_dict(), 'epoch':epoch}   
                torch.save(state, f'{model_pth_path}/tmp/Network_PlainSingleHead_epoch_{epoch}.pth')  # 保存DNN模型   
        ...
        test_loss_sum = np.sum(test_his1.reshape(-1, earlystopping_step), axis = 1)
        stop_index = int(earlystopping_step * np.argmin(test_loss_sum) + earlystopping_step / 2)
        IDT_best_pth_path = f'{model_pth_path}/early_stopping/IDT_model_best_stopat_{stop_index}_circ={self.circ}.pth'
        self.APART_args['best_ppth'] = IDT_best_pth_path
        self.APART_args['IDT_best_ppth'] = IDT_best_pth_path
        try:
            shutil.copy(f'{model_pth_path}/tmp/Network_PlainSingleHead_epoch_{stop_index}.pth', IDT_best_pth_path)
            shutil.rmtree(f'{model_pth_path}/tmp', ignore_errors=True); mkdirplus(f'{model_pth_path}/tmp') # 删除中间文件 
        except:
            self.TrainAnetLogger.warning(f"copy file error, {model_pth_path}/tmp/Network_PlainSingleHead_epoch_{stop_index}.pth not exist")

        # 保存实验结果
        np.savez(f'{model_loss_path}/LossHistory_IDT_circ={self.circ}.npz', # 保存DNN的loss
        ...
        self.APART_args['IDT_stop_epoch'] = int(stop_index)
        self.TrainAnetLogger.info(f"Network_PSRex: total time cost: {int(time.time() - t0)} s")
        ...
        t1 = time.time()
        input_dim, output_dim = self.APART_args['PSR_input_dim'], self.APART_args['PSR_output_dim']
        tmp_save_path = mkdirplus(f'{model_pth_path}/tmp')
        self.TrainAnetLogger.info(f'A -> PSR training has started...; circ = {self.circ}')
        Network_PSR = Network_PSR_PlainMLP(self.APART_args['PSR_layer_nums'], input_dim, self.APART_args['hidden_units'], output_dim).to(device)
        optimizer = optim.Adam(Network_PSR.parameters(), lr = self.APART_args['learning_rate'])
        # 第一次保存初始模型
        state = {'model':Network_PSR.state_dict()}   
        torch.save(state, f'{model_pth_path}/Network_PSR_initialized_circ={self.circ}.pth')  # 保存DNN模型
        epoch_nums = self.APART_args.get("PSR_epoch", self.APART_args['epoch'])
        criterion = nn.MSELoss()
        scheduler = optim.lr_scheduler.StepLR(optimizer, step_size = self.APART_args['lr_decay_step'], gamma = self.APART_args['lr_decay_rate'])
        
        epoch_index, train_his2, test_his2= [], [], []
        for epoch in range(epoch_nums):
            # 预测
            with torch.no_grad():
                x_dnn_test, psr_test = self.PSR_test_loader
                test_loss = Network_PSR.calculate_loss(x_dnn_test, psr_test)
                test_loss = float(test_loss.cpu().detach())
            # 训练
            psr_loss = 0
            for x_train_batch, psr_train_batch in self.PSR_train_loader:  # 按照 batch 进行训练
                # x_train_batch, psr_train_batch = x_train_batch.to(device), psr_train_batch.to(device)
                train_loss_batch = Network_PSR.calculate_loss(x_train_batch, psr_train_batch)
                optimizer.zero_grad()
                train_loss_batch.backward()
                optimizer.step()
                psr_loss += float(train_loss_batch.detach())
            
            scheduler.step()
            batch_num = len(self.PSR_train_loader)
            psr_loss /=  batch_num
            train_his2.append(psr_loss); test_his2.append(test_loss); epoch_index.append(epoch);           

            if epoch % 5 == 0:
                GPUtil.showUtilization()
                self.TrainAnetLogger.info(f"PSR training: epoch: {epoch}\t train loss: {psr_loss:.3e} "+
                                            f"test loss: {test_loss:.3e}"+ f"time cost: {int(time.time()-t0)} s   lr:{optimizer.param_groups[0]['lr']:.2e}")
                # 将梯度也打印出来
            if epoch % 100 == 0:
                for name, parms in Network_PSR.named_parameters():
                    if parms.requires_grad:
                        self.TrainAnetLogger.info(f"PSR training: epoch: {epoch}\t {name} grad: {parms.grad.max():.3e} {parms.grad.mean():.3e} {parms.grad.min():.3e}")
            if (epoch == 0) or ((epoch - 25) % 50 == 0):
                state = {'model':Network_PSR.state_dict(), 'optimizer':optimizer.state_dict(), 'epoch':epoch}   
                torch.save(state, f'{model_pth_path}/tmp/Network_PSR_epoch_{epoch}.pth')  # 保存DNN模型   
        # 构建 early stopping 必需的文件夹, 注意这里的 early stopping 是以 PSR 作为基准的，因为相比其他两个指标 PSR 更加重要
        if not os.path.exists(f'{model_pth_path}/early_stopping'): # 创建临时保存网络参数的文件夹
            os.mkdir(f'{model_pth_path}/early_stopping')
        train_his2, test_his2 = np.array(train_his2), np.array(test_his2)
        # early stopping 平均 earlystopping_step 的误差求最小
        earlystopping_step = min(epoch_nums, 50)
        test_loss_sum = np.sum(test_his2.reshape(-1, earlystopping_step), axis = 1)
        stop_index = int(earlystopping_step * np.argmin(test_loss_sum) + earlystopping_step / 2)
        PSR_best_pth_path = f'{model_pth_path}/early_stopping/PSR_model_best_stopat_{stop_index}_circ={self.circ}.pth'
        self.APART_args['PSR_best_ppth'] = PSR_best_pth_path
        try:
            shutil.copy(f'{model_pth_path}/tmp/Network_PSR_epoch_{stop_index}.pth', PSR_best_pth_path)
            shutil.rmtree(f'{model_pth_path}/tmp', ignore_errors=True); mkdirplus(f'{model_pth_path}/tmp') # 删除中间文件 
        except:
            self.TrainAnetLogger.warning(f"copy file error, {model_pth_path}/tmp/Network_PSR_PlainMLP_epoch_{stop_index}.pth not exist")

        # 保存实验结果
        np.savez(f'{model_loss_path}/LossHistory_PSR_circ={self.circ}.npz', # 保存DNN的loss
                epoch_index = epoch_index, 
                train_his = train_his2, 
                test_his = test_his2,
                stop_index = stop_index,
                circ = self.circ)
        self.APART_args['PSR_stop_epoch'] = int(stop_index)
        self.TrainAnetLogger.info(f"Network_PSR: total time cost: {int(time.time() - t1)} s")
        ...
        Network_PSRex = Network_PlainSingleHead(input_dim, hidden_units, output_dim).to(device)
        optimizer = torch.optim.Adam(Network_PSRex.parameters(), lr = self.APART_args['learning_rate'])
        scheduler = optim.lr_scheduler.StepLR(optimizer, step_size = self.APART_args['lr_decay_step'], gamma = self.APART_args['lr_decay_rate'])
        # 第一次保存初始模型
        state = {'model':Network_PSRex.state_dict(), 'optimizer':optimizer.state_dict(), 'epoch':0}
        torch.save(state, f'{tmp_save_path}/Network_PSRex_PSRextinction_epoch_{0}.pth')  # 保存DNN模型
        PSRex_epoch_nums = self.APART_args.get("PSRex_epoch", self.APART_args['epoch'])
        # 开始训练
        train_his3, test_his3 = [], []; criterion = nn.MSELoss()
        epoch_index = []
        for epoch in range(PSRex_epoch_nums):
            # 预测
            with torch.no_grad():
                x_dnn_test, psr_extinction_test = self.PSR_extinction_test_loader
                psr_extinction_pred = Network_PSRex.forward(x_dnn_test)
            ...
            for _, (x_train_batch, psr_extinction_train_batch) in enumerate(self.PSR_extinction_train_loader):  # 按照 batch 进行训练
                ...
                psr_extinction_train_batch_pred = Network_PSRex.forward(x_train_batch)
            ...
            scheduler.step()
            batch_num = len(self.PSR_extinction_train_loader)
            train_loss /= batch_num
            train_his3.append(train_loss); test_his3.append(test_loss); epoch_index.append(epoch);           
            ...
            if epoch % 5 == 0:
                ...
                self.TrainAnetLogger.info(f"epoch: {epoch}\t train loss: {train_loss:.3e} "+
                                            ...
                                            f"time cost: {int(time.time()-t0)} s   lr:{optimizer.param_groups[0]['lr']:.2e}")
            if (epoch == 0) or ((epoch - 25) % 50 == 0):
                state = {'model':Network_PSRex.state_dict(), 'optimizer':optimizer.state_dict(), 'epoch':epoch}   
                torch.save(state, f'{tmp_save_path}/Network_PSRex_PSRextinction_epoch_{epoch}.pth')  # 保存DNN模型  

        # 构建 early stopping 必需的文件夹, 注意这里的 early stopping 是以 IDT 作为基准的，因为相比其他两个指标 IDT 更加重要
        early_stopping_file = mkdirplus(f'{model_pth_path}/early_stopping')
        train_his3, test_his3 = np.array(train_his3), np.array(test_his3)

        # early stopping 平均 earlystopping_step 的误差求最小
        earlystopping_step = min(PSRex_epoch_nums, 50)
        test_loss_sum = np.sum(test_his3.reshape(-1, earlystopping_step), axis = 1)
        stop_index = int(earlystopping_step * np.argmin(test_loss_sum) + earlystopping_step / 2)
        ...
        try:
            shutil.copy(f'{tmp_save_path}/Network_PSRex_PSRextinction_epoch_{stop_index}.pth', PSR_extinction_best_pth_path)
            shutil.rmtree(tmp_save_path, ignore_errors=True); mkdirplus(tmp_save_path) # 删除中间文件 
        except:
            self.TrainAnetLogger.warning(f"copy file error, {tmp_save_path}/Network_PSRex_PSRextinction_epoch_{stop_index}.pth not exist")

        # 保存实验结果
        np.savez(f'{model_loss_path}/LossHistory_PSRextinction_circ={self.circ}.npz', # 保存DNN的loss
                epoch_index = epoch_index, 
                train_his = train_his3, 
                test_his = test_his3,
                stop_index = stop_index,
                circ = self.circ)
        self.APART_args['PSR_extinction_stop_epoch'] = int(stop_index)
        self.TrainAnetLogger.info(f"Network_PSRex: total time cost: {int(time.time() - t2)} s")

        return IDT_best_pth_path, PSR_best_pth_path, PSR_extinction_best_pth_path
    ...
    def DeePMO_train(self, concat_pre = False,  rm_tree = True, shrink_strategy = None, extract_strategy = None, 
                           ...
                           net_data_dirpath = "./data/APART_data/ANET_data", lock_json = False, PSR_ignition_Success_Threshold = None, 
                           **kwargs):
        if device is None:
        ...
        self.TrainAnetLogger.info(f"ANET_train_IDT_PSR: Device is {device}")
        ...
        PSR_ignition_Success_Threshold = self.APART_args.get('PSR_ignition_Success_Threshold', 250) if PSR_ignition_Success_Threshold is None else PSR_ignition_Success_Threshold
        self._TrainDataProcess(
            ...
            PSR_ignition_Success_Threshold = PSR_ignition_Success_Threshold,
            **kwargs
        )
        IDT_best_ppth, PSR_best_ppth, PSRex_best_ppth = self._DeePMO_train(device = device, PSR_train_outside_weight = PSR_train_outside_weight)
        ...
        self.TrainAnetLogger.info(f"{IDT_best_ppth=}")  
        self.TrainAnetLogger.info(f"{PSR_best_ppth=}")
        self.TrainAnetLogger.info(f"{PSRex_best_ppth=}")
        tmp_data = np.load(f"{self.model_loss_path}/LossHistory_IDT_circ={self.circ}.npz")
        train_his1 = tmp_data['train_his']; test_his1 = tmp_data['test_his']; stop_index1 = tmp_data['stop_index']
        tmp_data = np.load(f'{self.model_loss_path}/LossHistory_PSR_circ={self.circ}.npz')
        train_his2 = tmp_data['train_his']; test_his2 = tmp_data['test_his']; stop_index2 = tmp_data['stop_index']
        tmp_data = np.load(f'{self.model_loss_path}/LossHistory_PSRextinction_circ={self.circ}.npz')
        train_his3 = tmp_data['train_his']; test_his3 = tmp_data['test_his']; stop_index3 = tmp_data['stop_index']
        
        fig, ax = plt.subplots(1, 3, figsize = (16, 4))
        ax[0].semilogy(train_his1, lw=1, label='IDTtrain')
        ax[0].semilogy(test_his1, 'r', lw=1.2, label='IDTtest')
        ax[0].axvline(stop_index1, label = 'early stopping', color = 'green', )
        ax[0].set_xlabel('epoch'); 
        ax[0].legend(loc='upper right')
        ax[1].semilogy(train_his2, lw=1, label='PSRtrain')
        ax[1].semilogy(test_his2, 'r', lw=1.2, label='PSRtest')
        ax[1].axvline(stop_index2, label = 'early stopping', color = 'green')
        ax[1].set_xlabel('epoch'); 
        ax[1].set_ylabel('loss (log scale)')
        ax[1].legend(loc='upper right')
        ax[2].semilogy(train_his3, lw=1, label='PSRextinction_train')
        ax[2].semilogy(test_his3, 'r', lw=1.2, label='PSRextinction_PSRtest')
        ax[2].axvline(stop_index3, label = 'early stopping', color = 'green')
        ax[2].set_xlabel('epoch');
        ax[2].set_ylabel('loss (log scale)')
        ax[2].legend(loc='upper right')
        plt.tight_layout()
        plt.savefig(f'{self.model_path}/LossHistory_circ={self.circ}.png')
        plt.close(fig)

        self.TrainAnetLogger.info(f"Finished Train DNN! Total cost {time.time() - t0:.2f} s")


    def SkipSolveInverse(self, w1 = None, w2 = None, father_sample:str = None, save_dirpath = f'./inverse_skip', 
                             device = 'cpu', IDT_reduced_threshold = None, PSR_reduced_threshold = None,
                             PSR_ignition_Success_Threshold = 250, experiment_time = 15, **kwargs):
        """
        ...
        if not father_sample is None and os.path.exists(father_sample):
            ...
            inverse_alist = self.SortALIST(w1, w2, self.apart_data_path, experiment_time = experiment_time,
                    IDT_reduced_threshold = IDT_reduced_threshold, PSR_reduced_threshold = PSR_reduced_threshold, 
                    PSR_ignition_Success_Threshold = PSR_ignition_Success_Threshold)
        # 加载网络
        optim_idt_net = load_best_dnn(Network_PlainSingleHead, self.model_current_json, device = device, prefix = "IDT")
        optim_psr_net = load_best_dnn(Network_PSR_PlainMLP, self.model_current_json, device = 'cpu', DNN_instantiation_keys = (
                "PSR_best_ppth", "PSR_layer_nums", "PSR_input_dim", "hidden_units", "PSR_output_dim", 
            ))
        ...
        for index in range(experiment_time):
            ...
            try:  
                ...
                # 简化机理指标 vs 真实机理指标的绘图
                # IDT part
                cantera_idt_data, _ = yaml2idt(inverse_path + '/optim_chem.yaml', mode = self.IDT_mode, IDT_condition = self.IDT_condition, save = False, 
                ...
                final_idt = optim_idt_net(torch.tensor(A_init, dtype = torch.float32)).detach().numpy()
                ...
                # PSR part
                cantera_psr_data = yaml2psr(inverse_path + '/optim_chem.yaml', PSR_condition = self.PSR_condition, RES_TIME_LIST = self.RES_TIME_LIST,
                ...
                # DataNormalizationProcess scale
                final_psr = optim_psr_net(torch.tensor(A_init, dtype = torch.float32)).detach().numpy()
                final_psr = ReverseDataNormalizationProcess(final_psr, method = self.data_normalization_method,
                                        mean = psr_mean, std = psr_std, 
                                        replace_maskon_items = np.repeat(
                                                self.PSR_condition,
                                                repeats = 3,
                ...
                true_psr_data = self.true_psr_data.reshape(len(self.PSR_condition), -1); cantera_psr_data = cantera_psr_data.reshape(len(self.PSR_condition), -1);
                reduced_psr_data = self.reduced_psr_data.reshape(len(self.PSR_condition), -1)
                CompareDRON_PSR_lineplot(
                    detail_psr = true_psr_data,
                    reduced_psr = reduced_psr_data,
                    network_psr = final_psr,
                ...
                self.InverseLogger.info("Compare First PSR:" + "\n" + f"True:{true_psr_data[0]}; " + "\n" + f"Reduced:{reduced_psr_data[0]};\n" + 
                                        f"Cantera:{cantera_psr_data[0]};" + f"Final:{final_psr[0]};")
                self.InverseLogger.info("-" * 90)        
                
                # PSRex PART
                cantera_psrex_data = yaml2psr_extinction_time(inverse_path + '/optim_chem.yaml', PSR_condition = self.PSR_condition, RES_TIME_LIST = self.RES_TIME_LIST,
                ...
                np.savez(
                        ...
                        dnn_psr_data = np.array(final_psr),
                ...
                self.InverseLogger.info('plot compare idt picture done! time cost:%s seconds' % (time.time() - t1))
            except Exception:
    ...
    def gather_apart_data(self, save_path = "./data/APART_data/tmp", save_file_name = None,  rm_tree = True, 
            ...
            if cover or not os.path.exists(save_file_name) or os.path.getsize(save_file_name) /1024/1024 <= 1:
                ...
                return all_idt_data, all_T_data, all_psr_data, Alist_data, all_psr_extinction_data
            else:
    ...
    def SortALISTStat(self, apart_data_path = None, father_sample_path = None):
        ...
        network = load_best_dnn(Network_PlainSingleHead, self.model_current_json, device = 'cpu', prefix = "IDT")
        IDT_func = lambda x: network(torch.tensor(x, dtype = torch.float32)).detach().numpy()
        ...
        diff_previous_best_chem_PSR = (previous_best_chem_PSR - self.true_psr_data).reshape(len(self.PSR_condition), -1)
        # 找到 diff_previous_best_chem_PSR 中最大的一行
        diff_pbcS_max_index = np.argmax(np.sum(np.abs(diff_previous_best_chem_PSR), axis = 1))
        network = load_best_dnn(Network_PSR_PlainMLP, self.model_current_json, device = 'cpu', DNN_instantiation_keys = (
                "PSR_best_ppth", "PSR_layer_nums", "PSR_input_dim", "hidden_units", "PSR_output_dim", 
            ))
        PSR_func = lambda x: ReverseDataNormalizationProcess(network(torch.tensor(x, dtype = torch.float32)).detach().numpy(),
                                            method = self.data_normalization_method,
                                            mean = psr_mean, std = psr_std, 
                                            replace_maskon_items = np.repeat(
                                            self.PSR_condition,
                                            repeats = 3,
        ...
        true_psr_data = self.true_psr_data.reshape(len(self.PSR_condition), -1)[diff_pbcS_max_index]
        reduced_psr_data = self.reduced_psr_data.reshape(len(self.PSR_condition), -1)[diff_pbcS_max_index]
        trainNN_data = PSR_func(asamples)[:, diff_pbcS_max_index, :] - true_psr_data
        PSR = PSR.reshape(PSR.shape[0], len(self.PSR_condition), -1)[:, diff_pbcS_max_index, :]
        marker_psr = previous_best_chem_PSR.reshape(len(self.PSR_condition), -1)[diff_pbcS_max_index]
        sample_distribution_PSR(
                                PSR,
                                true_psr_data,
                                reduced_psr_data,
                                trainNN_data = trainNN_data,
                                asamples = asamples,
                                marker_psr = marker_psr,
                                # PSR_condition = [self.PSR_condition[diff_pbcS_max_index]],
                                # RES_TIME_LIST = self.RES_TIME_LIST,
...
@func_set_timeout(60)
def GenOneDataIDTPSR(IDT_condition: np.ndarray, PSR_condition: np.ndarray, RES_TIME_LIST: np.ndarray, Alist:list, 
                       eq_dict:dict, fuel:str, oxidizer: str, reduced_mech:str, index:int,  my_logger:Log, tmp_chem_path:str = None, 
                        IDT_mode = 0, remove_chem = True, idt_arrays:np.ndarray = 1, cut_time:np.ndarray = None, 
                        cut_time_alpha = 10, psr_error_tol = 50, save_path = 'data/APART_data/tmp', 
                        IDT_fuel:str = None, IDT_oxidizer: str = None, PSR_fuel:str = None, PSR_oxidizer: str = None,**kwargs):
    """
    将 IDT 和 PSR 结合起来
    ...
    PSR_T = _GenOnePSR(PSR_condition, RES_TIME_LIST, PSR_fuel, PSR_oxidizer, tmp_chem_path, index, 
                       my_logger, psr_error_tol = psr_error_tol, **kwargs)
    t1 = time.time()
    ...
    if isinstance(idt_T, int) or isinstance(PSR_T, int):
        return idt_T
    else:
        IDT, T = idt_T
        my_logger.info(f'cost {time.time()-t1:.2f} + {t1-t0:.2f} s for generate data of mechanism {index}, the first IDT element is {np.log10(IDT[0]):.3e}, the first PSR element is {PSR_T[0]:.3e}')
        np.savez(save_path, IDT = IDT, T = T, PSR = PSR_T, Alist = Alist)
        return None


@func_set_timeout(300)
def GenOneDataIDTPSR_plus_PSRex(IDT_condition: np.ndarray, PSR_condition: np.ndarray, RES_TIME_LIST: np.ndarray, Alist:list, 
                       eq_dict:dict, reduced_mech:str, index:int, my_logger:Log, tmp_chem_path:str = None, fuel:str = None, oxidizer: str = None, 
                        IDT_mode = 0, remove_chem = True, idt_arrays:np.ndarray = 1, cut_time:np.ndarray = None,
                        cut_time_alpha = 10, psr_error_tol = 50, save_path = 'data/APART_data/tmp', 
                        IDT_fuel:str = None, IDT_oxidizer: str = None, PSR_fuel:str = None, PSR_oxidizer: str = None,**kwargs):
    """
    ...
    # 将 PSR_condition RES_TIME_LIST 等转化为 array
    PSR_condition = np.array(PSR_condition); RES_TIME_LIST = np.array(RES_TIME_LIST); IDT_condition = np.array(IDT_condition)
    PSR_T, PSR_extinction = _GenOnePSR_plus_PSRex(PSR_condition, RES_TIME_LIST, PSR_fuel, PSR_oxidizer, tmp_chem_path, index, 
                                      my_logger, psr_error_tol = psr_error_tol, **kwargs)
    t1 = time.time()
    if IDT_fuel is None: IDT_fuel = fuel
    if IDT_oxidizer is None: IDT_oxidizer = oxidizer
    ...
    if isinstance(idt_T, int) or isinstance(PSR_T, int):
        ...
        IDT, T = idt_T
        my_logger.info(f"Mechanism {index} : cost {time.time()-t1:.2f} + {t1-t0:.2f} s, the first IDT element is {np.log10(IDT[0]):.3e}, the first PSR element is {PSR_T[0:3]}, the first PSR extinction time is {PSR_extinction[0:3]}, " + 
                       f"Memory usage: {psutil.Process(os.getpid()).memory_info().rss /1024 /1024 /1024:.2e} GB")
        np.savez(save_path, IDT = IDT, T = T, PSR = PSR_T, Alist = Alist, PSR_extinction = PSR_extinction)
        return None
