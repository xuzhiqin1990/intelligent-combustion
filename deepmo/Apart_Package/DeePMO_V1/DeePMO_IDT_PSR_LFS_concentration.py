...
from APART_base import _GenOneIDT, _GenOneLFS, _GenOnePSR_plus_PSRex, _GenOnePSR_concentration
...
from .DeePMO_V1_Network import Network_PlainTriple_PSR_PSRconcentration, Network_PlainSingleHead, DATASET_FourHead, DATASET_SingleHead
from APART_plot.Result_plot import CompareDRO_PSR_lineplot, CompareDRO_PSR_concentration_lineplot


class DeePMO_IDT_PSR_LFS_concentration(_DeePMO):
    """
        ...
        if GenASampleRange:
            if previous_best_chem is None:
                if self.circ == 0: 
                    ...
                    previous_best_chem, previous_best_chem_IDT, previous_best_chem_PSR, previous_best_chem_PSRex, previous_best_chem_LFS, previous_best_chem_PSR_concentration = self.SortALIST(
            ...
            self.GenASampleThreshold( 
                ...
                best_chem_PSR_concentration = previous_best_chem_PSR_concentration,best_chem_PSR = previous_best_chem_PSR, **kwargs
            )
        else:
    ...
    def GenASampleThreshold(self, best_chem_IDT, best_chem_LFS, best_chem_PSRex, best_chem_PSR_concentration,
                            best_chem_PSR, threshold_expand_factor = None, **kwargs):
        """
    ...
    def SortALIST(self, apart_data_path = None, experiment_time = 50, cluster_ratio = False, 
        ...
        w2 = self.PSR_weight
        w3 = self.LFS_weight
        w4 = self.PSR_concentration_weight
        ...
        diff_psr = np.linalg.norm(
            w2 / 500 * (self.true_psr_data - apart_data_psr), 
            axis = 1, ord = 2)
    ...
    def ASample(self, sample_size = None, coreAlist = None, passing_rate_upper_limit = None, if_LFS_filter:bool = True, if_PSRex_filter:bool = True,
        ...
        生成 ASample 的采样点; 采样点的生成方式为:
            1. 生成均匀采样点;
            2. 根据 IDT_weight, PSR_weight, LFS_weight, PSR_concentration_weight 对均匀采样点进行加权;
        ...
        if self.circ == 0 or self.circ == start_circ:
            ...
            net = load_best_dnn(Network_PlainTriple_PSR_PSRconcentration, self.model_previous_json, device = 'cpu', DNN_instantiation_keys = [
                    'best_ppth',
                'PSR_layer_nums',
                'input_dim',
                'hidden_units',
            ...
            PSR_reverse_DataNormalization = partial(ReverseDataNormalizationProcess, method = 'zscore',
            ...
            while len(self.samples) < sample_size:
                ...
                for A0 in coreAlist:
                    try:  
                        ...
                        self.GenAPARTDataLogger.info(f"IDT: Remain sample size is {len(tmp_sample)}; pass rate is {len(tmp_sample) / tmp_sample_size * 100} %, total size is {len(self.samples)}! cost {time.time() - t0:.2f}s; Memory usage: {psutil.Process(os.getpid()).memory_info().rss:.2e} B")
                        self.GenAPARTDataLogger.info("-·"*100)
                        tmp_sample, psr_pred_data = SampleAWithNet(net.forward_Net4, self.true_psr_data, threshold = self.psr_threshold, 
                                                    father_samples = tmp_sample, debug = True, reverse_DataNormalization = PSR_reverse_DataNormalization,)
                        if shrink_strategy:
                            tmp_boarder_sample = SampleAWithNet(net.forward_Net4, self.true_psr_data, threshold = self.psr_threshold + 1, 
                    ...
                    if if_PSRex_filter:
                        try:
                            ...
                            self.GenAPARTDataLogger.info(f"IDT + PSRex: Sampled {len(tmp_sample)} in this iter, pass Rate is {len(tmp_sample) / tmp_sample_size * 100} %, total size is {len(self.samples)}! cost {time.time() - t0:.2f}s; Memory usage: {psutil.Process(os.getpid()).memory_info().rss:.2e} B")  
    ...
    def _TrainDataProcess(self, load_file_name = None, concat_pre = False,  rm_tree = True, 
        ...
        if len(Alist_data.shape) == 1: 
            raise ValueError(f"Alist_data {Alist.shape} is 1D array, check the length of Alist_data is same or not")
        ...
        passed_psr_data, psr_mean, psr_std = DataNormalizationProcess(passed_psr_data, method = 'zscore', mask_on_threshold = np.amax(self.PSR_condition[:,1]))
        passed_T_data, T_mean, T_std = DataNormalizationProcess(passed_T_data, method = 'zscore',)
        ...
        self.APART_args['input_dim'], self.APART_args['output_dim'] = passed_A.shape[1], [passed_idt_data.shape[1], passed_lfs_data.shape[1], passed_psr_concentration_data.shape[1], passed_psr_data.shape[1]]
        ...
        dataset = DATASET_FourHead(
            ...
            data_QoI4 = torch.tensor(passed_psr_data, dtype = torch.float32),
            device = device
        )
        ...
        for x_test, idt_test, lfs_test, psr_concentration_test, psr_data in test_loader:
            self.test_loader = (x_test.to(device), idt_test.to(device), lfs_test.to(device), psr_concentration_test.to(device), psr_data.to(device))
    ...
    def _DeePMO_train(self, device = None, LFS_train_outside_weight = 1, **kwargs):
        ...
        ANET = Network_PlainTriple_PSR_PSRconcentration(self.APART_args['PSR_layer_nums'], input_dim, self.APART_args['hidden_units'], output_dim).to(device)
        ...
        torch.save(state, f'{model_pth_path}/Network_PlainTriple_PSR_PSRconcentration_initialized_circ={self.circ}.pth')  # 保存DNN模型
        ...
        for epoch in range(epoch_nums):
            # 预测
            with torch.no_grad():
                x_dnn_test, idt_test, lfs_test, psr_concentration_test, psr_test = self.test_loader
                # print(f'device of each: {x_dnn_test.device}, {idt_test.device}, {lfs_test.device}, {psr_concentration_test.device}, {psr_test.device}')
                # print(f'device of ANET.Network_PSR: {ANET.Network_PSR.device}')
                ...
                loss4 = ANET.Network_PSR.calculate_loss(x_dnn_test, psr_test)
                test_loss = [float(loss1.cpu().detach()), float(loss2.cpu().detach()), float(loss3.cpu().detach()), float(loss4.cpu().detach())]
            # 训练
            idt_loss, lfs_loss = 0, 0; concentration_loss = 0; psr_loss = 0
            for _, (x_train_batch, idt_train_batch, lfs_train_batch, concentration_batch, psr_batch) in enumerate(self.train_loader):  # 按照 batch 进行训练
                ...
                concentration_pred = ANET.forward_Net3(x_train_batch); psr_pred = ANET.forward_Net4(x_train_batch)
                ...
                loss4 = ANET.Network_PSR.calculate_loss(x_train_batch, psr_batch)

                train_loss_batch = loss1 / loss1.detach() + loss2 / loss2.detach() * LFS_train_outside_weight + loss3 / loss3.detach() + loss4 / loss4.detach()
                optimizer.zero_grad()
                train_loss_batch.backward()
                optimizer.step()
                idt_loss += float(loss1.detach()); lfs_loss += float(loss2.detach()); concentration_loss += float(loss3.detach()); psr_loss += float(loss4.detach())
            
            scheduler.step()
            batch_num = len(self.train_loader)
            idt_loss /=  batch_num; lfs_loss /=  batch_num; concentration_loss /= batch_num; psr_loss /= batch_num
            train_his1.append([idt_loss, lfs_loss, concentration_loss, psr_loss]); test_his1.append(test_loss); 
            epoch_index.append(epoch);           

            if epoch % 5 == 0:
                GPUtil.showUtilization()
                self.TrainAnetLogger.info(f"epoch: {epoch}\t train loss: {idt_loss:.3e} + {lfs_loss:.3e} + {concentration_loss:.3e}, + {psr_loss:.3e}"+
                                            f"test loss: {test_his1[-1][0]:.3e} + {test_his1[-1][1]:.3e} + {test_his1[-1][2]:.3e} + {test_his1[-1][3]:.3e}, " +
        ...
        test_loss_sum = np.sum(test_his1.reshape(-1, earlystopping_step, 4)[...,0], axis = 1)
    ...
    def DeePMO_train(self, concat_pre = False, rm_tree = True, shrink_strategy = None, extract_strategy = None, 
        ...
        ax[3].semilogy(train_his1[:,2], lw=1, label='PSRTtrain')
        ax[3].semilogy(test_his1[:,2], 'r', lw=1.2, label='PSRTtest')
    ...
    def SkipSolveInverse(self, father_sample:str = None, save_dirpath = f'./inverse_skip', 
        ...
        # 加载网络
        optim_net = load_best_dnn(Network_PlainTriple_PSR_PSRconcentration, self.model_current_json, device = device, DNN_instantiation_keys = [
                'best_ppth',
                'PSR_layer_nums',
                'input_dim',
                'hidden_units',
    ...
    def SortALISTStat(self, apart_data_path = None, father_sample_path = None):
        ...
        network = load_best_dnn(Network_PlainTriple_PSR_PSRconcentration, self.model_current_json, device = 'cpu', DNN_instantiation_keys = [
            'best_ppth',
                'PSR_layer_nums',
                'input_dim',
                'hidden_units',
...
def GenOneDataIDT_LFS_PSRex_concentration(IDT_condition: np.ndarray, PSR_condition: np.ndarray, RES_TIME_LIST: np.ndarray, LFS_condition: np.ndarray, PSR_concentration_kwargs: dict,
    ...
    if isinstance(idt_T, int) or isinstance(PSR_T, int) or isinstance(LFS, int) or isinstance(PSR_concentration, int):
        ...
        IDT, T = idt_T
        my_logger.info(f"Mechanism {index} : cost {t2 - t1:.2f} + {t1 - t0:.2f} + {t3 - t2:.2f} + {t4 - t3:.2f} s, len of IDT is {len(IDT)}, len of PSR is {len(PSR_T)}, len of LFS is {len(LFS)}, len of PSR_concentration is {len(PSR_concentration)}. \
                           Memory usage: {psutil.Process(os.getpid()).memory_info().rss /1024 /1024 /1024:.2e} GB")
