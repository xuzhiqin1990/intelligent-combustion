...
from APART_base import _GenOneIDT, _GenOneIDT_HRR, _GenOnePSR_with_extinction
...
from .DeePMO_V1_Network import Network_PlainDoubleHead, DATASET_DoubleHead, DATASET_SingleHead
from APART_plot.Result_plot import CompareDRO_PSR_lineplot
from Apart_Package.APART_base import gatherAPARTdata

class DeePMO_IDT_PSRex_HRR(_DeePMO):
    """
    在 DeePMO_IDT_PSRex 的基础上，增加了 HRR 的计算并全面取消了 PSR 的计算，使用 HRR 替换 PSR 的网络计算地位和预测地位
    """
        ...
        if GenASampleRange:
            if previous_best_chem is None:
                if self.circ == 0: 
                    ...
                    previous_best_chem, previous_best_chem_IDT, _, previous_best_chem_PSRex, previous_best_chem_HRR, _ = self.SortALIST(
            ...
            self.GenASampleThreshold( 
                best_chem_IDT = previous_best_chem_IDT, best_chem_HRR = previous_best_chem_HRR, best_chem_PSRex = previous_best_chem_PSRex,
                **kwargs
            )
        else:
    ...
    def cal_true_HRR(self, save_path = "./data/true_data/true_HRR.npz"):
        ...
        if save_path is not None and os.path.exists(save_path):
            ...
            _, self.true_hrr_data, _ = yaml2idt_hrr(
                self.detail_mech, self.IDT_condition, fuel = self.IDT_fuel, oxidizer = self.IDT_oxidizer, 
                cut_time = self.idt_cut_time
            )
            _, self.reduced_hrr_data, _ = yaml2idt_hrr(
    ...
    def GenASampleThreshold(self, best_chem_IDT, best_chem_HRR, best_chem_PSRex, threshold_expand_factor = None, **kwargs):
        """
        生成采样的阈值; 根据 best_chem_IDT, best_chem_HRR, best_chem_PSRex 生成采样阈值. 需要在 init 中就启动
        生成的具体策略为: 
            根据 best_chem_IDT/HRR/PSRex 中的每一项，计算其与真实值的误差 Rlos，乘以系数 threshold_expand_factor 后，
            以此作为 IDT/HRR/PSRex 采样阈值；同时，如果 thresholds 是一个列表，将阈值设置为
            Rlos * threshold_expand_factor * thresholds[i] / thresholds[i - 1]
        add
            self.idt_threshold, self.hrr_threshold, self.psr_extinction_threshold
        """
        self.GenAPARTDataLogger.info(f"Start GenASampleThreshold with best_chem_IDT: {best_chem_IDT}, best_chem_PSR: {best_chem_HRR}, best_chem_PSRex: {best_chem_PSRex}")
        ...
        self.GenAPARTDataLogger.info(f"Current CIRC Rlos are IDT:{idt_Rlos} ~ HRR:{hrr_Rlos} ~ PSRex:{psrex_Rlos}")
        if self.circ >= 1:
        ...
        self.GenAPARTDataLogger.info(f"Current CIRC thresholds are fixed as {self.idt_threshold} ~ {self.hrr_threshold} ~ {self.psrex_threshold}")
        self.GenAPARTDataLogger.info("=" * 50)
        # self.WriteCurrentAPART_args(idt_threshold = self.idt_threshold, hrr_threshold = self.hrr_threshold, psrex_threshold = self.psrex_threshold)


    def SortALIST(self, w1 = None, w2 = None, w3 = None, apart_data_path = None, experiment_time = 50, cluster_ratio = False, 
        ...
        apart_data_psrex = apart_data['all_psr_extinction_data']; apart_data_hrr = apart_data['all_hrr_data']
        ...
        diff_idt = diff_idt[if_idt_pass]; diff_psrex = diff_psrex[if_idt_pass]; diff_hrr = diff_hrr[if_idt_pass]
        ...
        diff = diff_idt + diff_psrex * w2[0] + diff_hrr
        ...
        apart_data_T = apart_data_T[index,:]; apart_data_hrr = apart_data_hrr[index,:]
        ...
        if return_all:
            return Alist[0:experiment_time, :], apart_data_idt[0:experiment_time, :] , apart_data_psr[0:experiment_time, :], apart_data_psrex[0:experiment_time, :], apart_data_hrr[0:experiment_time, :], apart_data_T[0:experiment_time, :]
    ...
    def ASample(self, sample_size = None, coreAlist = None, IDT_weight = None, PSRex_weight = None, HRR_weight = None,
                passing_rate_upper_limit = None, if_HRR_filter:bool = True, if_PSRex_filter:bool = True,
        ...
        # 预设置
        self.GenAPARTDataLogger.info(f"Start The ASample Process; Here we apply three aspect into consideration: IDT: True, HRR: {if_HRR_filter} and PSRextinction: {if_PSRex_filter}")
        # 提取采样的左右界限 + 采样阈值
        ...
        # 检测类中是否存在 self.idt_threshold, self.hrr_threshold, self.psrex_threshold
        if not hasattr(self, 'idt_threshold') or not hasattr(self, 'hrr_threshold') or not hasattr(self, 'psrex_threshold'):
            idt_threshold = self.APART_args['idt_threshold']
            hrr_threshold = self.APART_args['hrr_threshold']
            psrex_threshold = self.APART_args['psrex_threshold']
        ...
        if self.circ == 0 or self.circ == start_circ:
            ...
            psrex_net = load_best_dnn(ANET_PSRextinction, self.model_previous_json, device = 'cpu', prefix = "PSR_extinction")
            # 将 true psr data 和 hrr_threshold 标准化
            psr_mean = np.array(json_data['psr_mean']); psr_std = np.array(json_data['psr_std'])
            # true_psr = (self.true_psr_data - psr_mean) / psr_std
            # hrr_threshold = hrr_threshold / psr_std
            ...
            if coreAlist is None:
                cluster_weight = kwargs.get('cluster_weight', 0.1)
                previous_coreAlist = self.SortALIST(IDT_weight, PSRex_weight, HRR_weight, 
                        apart_data_path = os.path.dirname(self.apart_data_path) + f'/apart_data_circ={self.circ - 1}.npz',
            ...
            t0 = time.time(); idt_zero_father_sample_times = 0; hrr_zero_father_sample_times = 0; psrex_zero_father_sample_times = 0
            ...
            while len(self.samples) < sample_size:
                ...
                for A0 in coreAlist:
                    ...
                    if if_PSRex_filter:
                        try:
                            ...
                            # self.GenAPARTDataLogger.info(f"PSR: On Average, Working Condition which not satisfy threshold IS {np.unique(np.tile(self.PSR_condition, (1,3))[np.where(hrr_Rlos > true_psr)[0],:], axis = 0)}")
                            ...
                            self.GenAPARTDataLogger.info(f"IDT + PSRex: Sampled {len(tmp_boarder_sample)} Boarder Data in this iter, total size is None! Memory usage: {psutil.Process(os.getpid()).memory_info().rss /1024 /1024 /1024:.2e} GB")  
                            self.GenAPARTDataLogger.info("——"*100)
                            if len(tmp_sample) == 0:
                                raise ValueError("tmp_sample is Empty!")
                        except ValueError or IndexError:
                    ...
                    if if_HRR_filter:
                        try:
                            ...
                            self.GenAPARTDataLogger.info(f"IDT + PSRex + HRR: Remained {len(tmp_sample)} in this iter, pass Rate is {len(tmp_sample) / tmp_sample_size * 100} %, total size is {len(self.samples)}! cost {time.time() - t0:.2f}s; Memory usage: {psutil.Process(os.getpid()).memory_info().rss:.2e} B")  
                            self.GenAPARTDataLogger.info(f"IDT + PSRex + HRR: Sampled {len(tmp_boarder_sample)} Boarder Data in this iter, total size is None! Memory usage: {psutil.Process(os.getpid()).memory_info().rss / 1024 / 1024 / 1024:.2e} GB")  
                            self.GenAPARTDataLogger.info("-·"*100)
                            if len(tmp_sample) == 0:
                                raise ValueError("tmp_sample is Empty!")
                    ...
                    if pass_rate ** 1/3 > passing_rate_upper_limit * 0.9 and pass_rate  ** 1/3 > 0.5:
                        ...
                        self.GenAPARTDataLogger.warning(f"IDT + PSRex + HRR: pass_rate is {pass_rate * 100} %, which is too high! idt_threshold have changed to log10({self.idt_threshold}) = {np.log10(self.idt_threshold)}; psrex_threshold have changed to log2({self.psrex_threshold}) = {np.log2(self.psrex_threshold)}")
                    
                    # 样本太多立即退出
                    if len(self.samples) >= sample_size * 1.2:
                        self.GenAPARTDataLogger.warning(f"IDT + PSRex + HRR: Stop the Sampling Process, Total Size has come up to {len(self.samples)} data after this iter! cost {time.time() - t0:.2f}s")  
                ...
                if time.time() - t0 > sample_size * average_timeout_time:
                    ...
                    break
                self.GenAPARTDataLogger.info(f"In this Iteration, IDT + PSRex + HRR: Total Size has come up to {len(self.samples)} data after this iter! cost {time.time() - t0:.2f}s")  
                self.GenAPARTDataLogger.info(f"Memory usage: {psutil.Process(os.getpid()).memory_info().rss /1024 /1024 /1024} GB")
        ...
        # 更新 idt_threshold 和 hrr_threshold 和 psrex_threshold
    ...
    def GenDataFuture(self, samples:np.ndarray = None, idt_cut_time_alpha = 1.5, psr_error_tol = None, 
        ...
        with ProcessPoolExecutor(max_workers = cpu_process) as exec:
            for index in range(sample_length):
                try:
                    future = exec.submit(
                            GenOneDataIDT_HRR_PSRextinction, 
    ...
    def _TrainDataProcess(self, load_file_name = None, concat_pre = False,  rm_tree = True, 
        ...
        if isinstance(load_file_name, str) and not concat_pre:
            ...
            all_hrr_data = Data['all_hrr_data']
            # 判断是否加载之前的 data:
        else:
            all_idt_data, all_T_data, Alist_data, all_psr_data, all_hrr_data = [], [], [], [], []
            ...
            for file in load_file_name:   
                ...
                all_hrr_data.extend(Data['all_hrr_data'])
        ...
        if extract_strategy and self.circ != 0:
            ...
            for circ in range(self.circ):
                ...
                if os.path.exists(apart_data_path) and os.path.getsize(apart_data_path) /1024 /1024 > 1:
                    Alist, IDT, PSR, PSRex, HRR, T = self.SortALIST(
        ...
        all_idt_data, all_T_data, all_psr_data, Alist_data, all_psr_extinction_data, all_hrr_data  = \
             np.log10(all_idt_data), np.array(all_T_data), np.array(all_psr_data), np.array(Alist_data), np.log2(all_psr_extinction_data), np.log10(all_hrr_data)
        ...
        self.TrainAnetLogger.info(f"The size of all_idt_data is {all_idt_data.shape}, all_T_data is {all_T_data.shape}, \n all_psr_data is {all_psr_data.shape}, Alist_data is {Alist_data.shape}, all_psr_extinction_data is {all_psr_extinction_data.shape}, all_hrr_data is {all_hrr_data.shape}")
        ...
        passed_idt_data, passed_T_data, passed_psr_data, passed_A, passed_hrr_data = \
            all_idt_data[if_pass], all_T_data[if_pass], all_psr_data[if_pass], Alist_data[if_pass], all_hrr_data[if_pass]
        ...
        if passed_idt_data.shape[0] > expected_sample_size:
            passed_idt_data, passed_T_data, passed_psr_data, passed_A, passed_hrr_data = \
                ...
                passed_psr_data[:expected_sample_size], passed_A[:expected_sample_size], passed_hrr_data[:expected_sample_size]
        ...
        assert passed_hrr_data.shape[0] == passed_idt_data.shape[0] == passed_T_data.shape[0] == passed_psr_data.shape[0] == passed_A.shape[0], \
            f"Error The shape at axis 0 is not same! passed_hrr_data.shape = {passed_hrr_data.shape}, passed_idt_data.shape = {passed_idt_data.shape}, passed_T_data.shape = {passed_T_data.shape}, passed_psr_data.shape = {passed_psr_data.shape}, passed_A.shape = {passed_A.shape}"
        ...
        self.APART_args['input_dim'], self.APART_args['output_dim'] = passed_A.shape[1], [passed_idt_data.shape[1], passed_hrr_data.shape[1]]
        ...
        dataset = DATASET_DoubleHead(
            ...
            data_QoI2 = torch.tensor(passed_hrr_data, dtype = torch.float32),
            device = device
        )
    ...
    def _DeePMO_train(self, device = None, HRR_train_outside_weight = 1, **kwargs):
        ...
        ANET = ANET_PSRextinction(input_dim, hidden_units, output_dim).to(device)
        ...
        torch.save(state, f'{tmp_save_path}/ANET_PSRextinction_epoch_{0}.pth')  # 保存DNN模型
        # 开始训练
        ...
        for epoch in range(epoch_nums):
            ...
            if (epoch == 0) or ((epoch - 25) % 50 == 0):
                state = {'model':ANET.state_dict(), 'optimizer':optimizer.state_dict(), 'epoch':epoch}   
                torch.save(state, f'{tmp_save_path}/ANET_PSRextinction_epoch_{epoch}.pth')  # 保存DNN模型  
        ...
        try:
            shutil.copy(f'{tmp_save_path}/ANET_PSRextinction_epoch_{stop_index}.pth', PSR_extinction_best_pth_path)
            shutil.rmtree(tmp_save_path, ignore_errors=True); mkdirplus(tmp_save_path) # 删除中间文件 
        except:
            self.TrainAnetLogger.warning(f"copy file error, {tmp_save_path}/ANET_PSRextinction_epoch_{stop_index}.pth not exist")

        # 保存实验结果
        np.savez(f'{model_loss_path}/ANET_PSRextinction_circ={self.circ}.npz', # 保存DNN的loss
                epoch_index = epoch_index, 
    ...
    def DeePMO_train(self, concat_pre = False, rm_tree = True, shrink_strategy = None, extract_strategy = None, 
        ...
        tmp_data = np.load(f'{self.model_loss_path}/ANET_PSRextinction_circ={self.circ}.npz')
        ...
        # ax[2].semilogy(train_his1[:,2], lw=1, label='Ttrain')
        # ax[2].semilogy(test_his1[:,2], 'r', lw=1.2, label='Ttest')
    ...
    def SkipSolveInverse(self, w1 = None, w2 = None, w3 = None, father_sample:str = None, save_dirpath = f'./inverse_skip', 
        ...
        if not father_sample is None and os.path.exists(father_sample):
            ...
            inverse_alist = self.SortALIST(w1, w2, w3, self.apart_data_path, experiment_time = experiment_time,
        ...
        optim_psrex_net = load_best_dnn(ANET_PSRextinction, self.model_current_json, device = device, prefix = "PSR_extinction")
        for index in range(experiment_time):
            ...
            try:  
                ...
                # 简化机理指标 vs 真实机理指标的绘图
                # IDT part
                cantera_idt_data, cantera_hrr_data, _ = yaml2idt_hrr(inverse_path + '/optim_chem.yaml', mode = self.IDT_mode, IDT_condition = self.IDT_condition, save = False, 
        ...
        if cover or not os.path.exists(save_file_name) or os.path.getsize(save_file_name) /1024/1024 <= 1:
            ...
            for target_file in files:
                ...
                if os.path.getsize(target_file) >= 5:
                    ...
                    all_hrr_data.append(tmp['HRR'].tolist())
    ...
    def SortALISTStat(self, apart_data_path = None, father_sample_path = None):
        ...
        # 使用 sample_distribution_PSR 来展示 HRR 的结果
        ...
        network = load_best_dnn(ANET_PSRextinction, self.model_current_json, device = 'cpu', prefix = "PSR_extinction")
        ...
        logger.info("IDT, PSR, PSRex, HRR 的分布情况已保存至 ./analysis/ 中")
    ...
    def IDT_SensitivityDesent(self, start_samples = None, target_IDT_conditions = None, iter_nums = 10, delta = 0.01,  
        ...
        """
        np.set_printoptions(precision = 6, suppress = True)
        dirpath = mkdirplus("./data/SensitivityDesent_IDT")

        cpu_nums = cpu_nums if cpu_nums is not None else os.cpu_count() - 1
        father_samples = start_samples if start_samples is not None else [self.A0]
        logger = Log("./log/SensitivityDesent.log")
        father_sample_size = len(father_samples) if father_sample_size is None else father_sample_size
        if target_IDT_conditions is None: 
            target_IDT_conditions = self.IDT_condition
            benchmark_IDT = self.true_idt_data
        else:
        ...
        for iter in range(iter_nums):
            ...
            for i, sample in enumerate(father_samples):
                try:
                    ...
                    # 计算损失函数的梯度
                    print(base_IDT, benchmark_IDT, IDT_sensitivity_list.shape)
                    ...
                    for j in range(eve_sample_size):
                        desent_samples.append((sample + j / 10 * (desent_sample - sample)).tolist())
                    Desent_samples.append(desent_samples)
                    logger.info(f"iter {iter} finished, sampled {len(Desent_samples) * len(Desent_samples[0])} points.")
                    logger.info(f"The example of base_IDT is {base_IDT}, the benchmark_IDT is {benchmark_IDT}, desent step is {step * grad}.")
                except:
                    print(traceback.format_exc())
                    print(f'iter {iter} failed, sample {i} failed. Skip it.')
                    continue
                DDesent_samples.append(Desent_samples)
                father_samples = np.array(Desent_samples).reshape(-1, len(sample))
                np.random.shuffle(father_samples)
        ...
        # 将 DDsent_samples 转化为二维数组, 其中一个维度是 sample 的维度
        DDesent_samples = np.array(DDesent_samples).reshape(-1, len(sample))
        np.save("./data/SensitivityDesent_IDT/Desent_samples.npy", DDesent_samples, )
        logger.info(f"DDesent_samples has been saved to ./data/SensitivityDesent_IDT/Desent_samples.npy; shape is {DDesent_samples.shape}")
        # 使用多进程计算所有的 IDT 值
        mkdirplus("./data/SensitivityDesent_IDT/tmp")
        with ProcessPoolExecutor(max_workers = cpu_nums) as exec:
            for index, sample in enumerate(DDesent_samples):
                exec.submit(
                        GenOneDataIDT_HRR_PSRextinction, 
                        ...
                        cut_time_alpha = 10,
                        error_tol = 0, 
                        PSR_EXP_FACTOR = self.PSRex_decay_rate,
                        save_path = "./data/SensitivityDesent_IDT/tmp",
                        my_logger = logger,
                        )
        # 使用 gather_data 函数将数据收集起来
        gatherAPARTdata(npz_keys = ['IDT', 'T', 'PSR_T', 'PSR_extinction', 'HRR'], 
                        apart_data_keys = ['idt', 'T', 'psr', 'psr_extinction', 'hrr'],
                        save_path = "./data/SensitivityDesent_IDT/tmp", rm_tree = True, 
                        save_file_name = f"./data/SensitivityDesent_IDT/SensitivityDesentData.npz",
                        cpu_nums = os.cpu_count() - 1, logger = logger
                        )
        # 加载 "./data/SensitivityDesentData.npz" 
        data = np.load("./data/SensitivityDesent_IDT/SensitivityDesentData.npz"); Alist = data['Alist']; all_idt_data = data['all_idt_data']
        # 将 data['all_psr_extinction_data'] - self.true_psr_extinction_data 的绝对值计算无穷范数，得到的是一个一维数组
        ...
        # 将这 10 个 Alist 保存到 "./data/SensitivityDesentData.npz" 中
        np.savez("./data/SensitivityDesent_IDT/SensitivityDesentData_filtered.npz", Alist = Alist, all_idt_data = all_idt_data)
        return Alist, all_idt_data


@func_set_timeout(300)
def GenOneDataIDT_HRR_PSRextinction(IDT_condition: np.ndarray, PSR_condition: np.ndarray, RES_TIME_LIST: np.ndarray, Alist:list, 
    ...
    idt_T = _GenOneIDT_HRR(IDT_condition, IDT_fuel, IDT_oxidizer, tmp_chem_path,  index,  my_logger, IDT_mode,
                     idt_arrays, cut_time, cut_time_alpha, **kwargs)
    ...
    if isinstance(idt_T, int) or isinstance(PSR_T, int):
        ...
        np.savez(save_path, IDT = IDT, T = T, PSR = PSR_T, Alist = Alist, PSR_extinction = PSR_extinction, HRR = HRR)
        return None
...
class DATASET_IDT_PSRextinction(Dataset):
    """
    自定义的 dataset，要求是输入和输出（均是 tensor）的第一维度是数据长度且相同
    
    如果使用索引的话会返回4个值，分别是 A, IDT, PSRextinction
    """
    def __init__(self, data_A:torch.Tensor, data_psr_extinction:torch.Tensor) -> None:
        super().__init__()
        self.data_A = data_A; self.data_psr_extinction = data_psr_extinction
    
    def __len__(self,):
        assert len(self.data_A) == len(self.data_psr_extinction)
        return len(self.data_A)

    def __getitem__(self, index):
        return self.data_A[index], self.data_psr_extinction[index]
    
    def xshape(self, dim = None):
        if not dim is None:
            return self.data_A.shape[dim]
        return self.data_A.shape

    
class ANET_PSRextinction(nn.Module):
    """
    专属版优化 PSRextinction 的网络
    具体结构为 input - 3000 - 2000 - 2000 - output
    """
    def __init__(self, input_dim:int, 
        ...
        # 定义隐藏层
        super().__init__()
        self.flatten1 = nn.Linear(input_dim, hidden_units[0], bias = False)
        self.flatten2 = nn.Linear(hidden_units[0], hidden_units[1], bias = False)
        self.flatten3 = nn.Linear(hidden_units[1], hidden_units[2], bias = False)
        self.flatten4 = nn.Linear(hidden_units[2], output_dim, bias = False)
        self.flatten_activation = nn.ReLU()

        # 使用Kaiming初始化
        nn.init.kaiming_normal_(self.flatten1.weight)
        nn.init.kaiming_normal_(self.flatten2.weight)
        nn.init.kaiming_normal_(self.flatten3.weight)
    
    def forward(self, x):
        x = self.flatten_activation(self.flatten1(x))
        x = self.flatten_activation(self.flatten2(x))
        x = self.flatten_activation(self.flatten3(x))
        x = self.flatten4(x)
        return x
    

class DATASET_IDT_HRR(Dataset):
    """
    自定义的 dataset，要求是输入和输出（均是 tensor）的第一维度是数据长度且相同
    
    如果使用索引的话会返回4个值，分别是 A, IDT, HRR, T
    """
    def __init__(self, data_A:torch.Tensor, data_idt:torch.Tensor, data_hrr:torch.Tensor) -> None:
        super().__init__()
        self.data_A = data_A; self.data_idt = data_idt; self.data_hrr = data_hrr
    
    def __len__(self,):
        assert len(self.data_A) == len(self.data_idt)
        return len(self.data_A)

    def __getitem__(self, index):
        return self.data_A[index], self.data_idt[index], self.data_hrr[index]
