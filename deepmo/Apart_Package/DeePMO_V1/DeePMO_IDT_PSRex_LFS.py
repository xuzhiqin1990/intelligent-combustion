...
from APART_base import _GenOneIDT, _GenOneLFS, _GenOnePSR_with_extinction
...
from .DeePMO_V1_Network import Network_PlainDoubleHead, Network_PlainSingleHead, DATASET_DoubleHead, DATASET_SingleHead
from APART_plot.Result_plot import CompareDRO_PSR_lineplot


class DeePMO_IDT_PSRex_LFS(_DeePMO):
    """
    在 DeePMO_IDT_PSRex 的基础上，增加了 LFS 的计算并全面取消了 PSR 的计算，使用 LFS 替换 PSR 的网络计算地位和预测地位
    """
    def __init__(self, circ = 0, basic_set = True, GenASampleRange = True, setup_file: str = './settings/setup.yaml', 
        ...
        if GenASampleRange:
            if previous_best_chem is None:
                if self.circ == 0: 
                    ...
                    previous_best_chem, previous_best_chem_IDT, _, previous_best_chem_PSRex, previous_best_chem_LFS, _ = self.SortALIST(
        ...
        assert self.IDT_mode is not None, "IDT_mode is None, please check the setup file or set IDT_mode manually"
    ...
    def GenASampleThreshold(self, best_chem_IDT, best_chem_LFS, best_chem_PSRex, threshold_expand_factor = None, **kwargs):
        """
        ...
        lfs_Rlos = np.amax(np.abs(np.log10(best_chem_LFS) - np.log10(self.true_lfs_data)))
        psrex_Rlos = PSRex_shrink_error(np.log2(best_chem_PSRex), np.log2(self.true_extinction_time))
        self.GenAPARTDataLogger.info(f"Current CIRC Rlos are IDT:{idt_Rlos} ~ LFS:{lfs_Rlos} ~ PSRex:{psrex_Rlos}")
        if self.circ >= 1:
        ...
        self.GenAPARTDataLogger.info(f"Current CIRC thresholds are fixed as {self.idt_threshold} ~ {self.lfs_threshold} ~ {self.psrex_threshold}")
    ...
    def SortALIST(self, w1 = None, w2 = None, w3 = None, apart_data_path = None, experiment_time = 50, cluster_ratio = False, 
        ...
        diff_idt = diff_idt[if_idt_pass]; diff_psrex = diff_psrex[if_idt_pass]; diff_lfs = diff_lfs[if_idt_pass]
        ...
        diff = diff_idt + diff_psrex * w2[0] + diff_lfs 
        ...
        if return_all:
            return Alist[0:experiment_time, :], apart_data_idt[0:experiment_time, :] , apart_data_psr[0:experiment_time, :], apart_data_psrex[0:experiment_time, :], apart_data_lfs[0:experiment_time, :], apart_data_T[0:experiment_time, :]
    ...
    def ASample(self, sample_size = None, coreAlist = None, IDT_weight = None, PSRex_weight = None, LFS_weight = None,
        ...
        202230730: 增加了通过率上限 passing_rate_upper_limit, 是否启用 PSR 和 PSRex 训练的 if_LFS_filter 与 if_PSRex_filter
        """
        ...
        if not hasattr(self, 'idt_threshold') or not hasattr(self, 'lfs_threshold') or not hasattr(self, 'psrex_threshold'):
            idt_threshold = self.APART_args['idt_threshold']
            lfs_threshold = self.APART_args['lfs_threshold']
            psrex_threshold = self.APART_args['psrex_threshold']
        ...
        if self.circ == 0 or self.circ == start_circ:
            ...
            if coreAlist is None:
                cluster_weight = kwargs.get('cluster_weight', 0.1)
                previous_coreAlist = self.SortALIST(IDT_weight, PSRex_weight, LFS_weight, 
                        apart_data_path = os.path.dirname(self.apart_data_path) + f'/apart_data_circ={self.circ - 1}.npz',
            ...
            t0 = time.time(); idt_zero_father_sample_times = 0; lfs_zero_father_sample_times = 0; psrex_zero_father_sample_times = 0
                    ...
                    if if_LFS_filter:
                        try:
                            tmp_sample, lfs_pred_data = SampleAWithNet(net.forward_Net2, self.true_lfs_data, threshold = self.lfs_threshold, 
                                                        ...
                                                        reduced_data = self.reduced_lfs_data, reduced_threshold = None,
                                                        )
                            if shrink_strategy:
    ...
    def _TrainDataProcess(self, load_file_name = None, concat_pre = False,  rm_tree = True, 
        ...
        if isinstance(load_file_name, str) and not concat_pre:
            ...
            all_idt_data, all_T_data, Alist_data, all_psr_data, all_lfs_data = [], [], [], [], []
        ...
        if extract_strategy and self.circ != 0:
            ...
            for circ in range(self.circ):
                ...
                if os.path.exists(apart_data_path) and os.path.getsize(apart_data_path) /1024 /1024 > 1:
                    Alist, IDT, PSR, PSRex, LFS, T = self.SortALIST(
        ...
        passed_idt_data, passed_T_data, passed_psr_data, passed_A, passed_lfs_data = \
            all_idt_data[if_pass], all_T_data[if_pass], all_psr_data[if_pass], Alist_data[if_pass], all_lfs_data[if_pass]
        ...
        if passed_idt_data.shape[0] > expected_sample_size:
            passed_idt_data, passed_T_data, passed_psr_data, passed_A, passed_lfs_data = \
                ...
                passed_psr_data[:expected_sample_size], passed_A[:expected_sample_size], passed_lfs_data[:expected_sample_size]
        ...
        self.APART_args['input_dim'], self.APART_args['output_dim'] = passed_A.shape[1], [passed_idt_data.shape[1], passed_lfs_data.shape[1]]
        ...
        for x_test, idt_test, lfs_test in test_loader:
            self.test_loader = (x_test.to(device), idt_test.to(device), lfs_test.to(device))
    ...
    def _DeePMO_train(self, device = None, LFS_train_outside_weight = 1, **kwargs):
        ...
        self.TrainAnetLogger.info(f'A -> IDT & LFS training has started...; circ = {self.circ}')
        ...
        for epoch in range(epoch_nums):
            # 预测
            with torch.no_grad():
                x_dnn_test, idt_test, lfs_test = self.test_loader
                idt_pred, lfs_pred = ANET.forward_Net1(x_dnn_test), ANET.forward_Net2(x_dnn_test)
                loss1 = criterion(idt_pred, idt_test); loss2 = criterion(lfs_pred, lfs_test)
                test_loss = [float(loss1.cpu().detach()), float(loss2.cpu().detach())]
            # 训练
            idt_loss, lfs_loss = 0, 0
            for _, (x_train_batch, idt_train_batch, lfs_train_batch) in enumerate(self.train_loader):  # 按照 batch 进行训练
                ...
                train_loss_batch = loss1 / loss1.detach() + loss2 / loss2.detach() * LFS_train_outside_weight
                optimizer.zero_grad()
                train_loss_batch.backward()
                optimizer.step()
                idt_loss += float(loss1.detach()); lfs_loss += float(loss2.detach())
            
            scheduler.step()
            batch_num = len(self.train_loader)
            idt_loss /=  batch_num; lfs_loss /=  batch_num
            train_his1.append([idt_loss, lfs_loss]); test_his1.append(test_loss)
            epoch_index.append(epoch);           

            if epoch % 5 == 0:
                GPUtil.showUtilization()
                self.TrainAnetLogger.info(f"epoch: {epoch}\t train loss: {idt_loss:.3e} + {lfs_loss:.3e} "+
    ...
    def SkipSolveInverse(self, w1 = None, w2 = None, w3 = None, father_sample:str = None, save_dirpath = f'./inverse_skip', 
        ...
        for index in range(experiment_time):
            ...
            try:  
                ...
                psr_extinction_data = apart_data['all_psr_extinction_data']; lfs_data = apart_data['all_lfs_data'];
                # 查找 Ainit 的 index
                index = np.where(np.all(Alist_data == A_init, axis = 1))[0][0] 
                cantera_idt_data = idt_data[index]; cantera_psr_data = psr_data[index]; cantera_psrex_data = psr_extinction_data[index]; cantera_lfs_data = lfs_data[index]
                # 简化机理指标 vs 真实机理指标的绘图
                # IDT part
...
def GenOneDataIDT_LFS_PSRextinction(IDT_condition: np.ndarray, PSR_condition: np.ndarray, RES_TIME_LIST: np.ndarray, LFS_condition: np.ndarray, Alist:list, 
    ...
    idt_T = _GenOneIDT(IDT_condition, IDT_fuel, IDT_oxidizer, tmp_chem_path,  index,  my_logger, IDT_mode,
    ...
    if isinstance(idt_T, int) or isinstance(PSR_T, int) or isinstance(LFS, int):
        return idt_T
    else:
        IDT, T = idt_T
        my_logger.info(f"Mechanism {index} : cost {t2 - t1:.2f} + {t1 - t0:.2f} + {time.time() - t2:.2f} s, the first IDT element is {np.log10(IDT[0]):.3e}, the first PSR element is {PSR_T[0:3]}, the first PSR extinction time is {PSR_extinction[0:3]}, " + 
                       f"first LFS element is {LFS[0]:.3e}, Memory usage: {psutil.Process(os.getpid()).memory_info().rss /1024 /1024 /1024:.2e} GB")
        np.savez(save_path, IDT = IDT, T = T, PSR = PSR_T, Alist = Alist, PSR_extinction = PSR_extinction, LFS = LFS)
        return None
