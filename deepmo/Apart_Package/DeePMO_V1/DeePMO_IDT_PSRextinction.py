...
from .basic_set import _DeePMO, PSRex_shrink_error, gatherAPARTdata
from APART_base import _GenOneIDT,  _GenOnePSR_with_extinction
...
from .DeePMO_V1_Network import Network_PlainSingleHead, Network_PlainDoubleHead, DATASET_TripleHead, DATASET_SingleHead
from .DeePMO_IDT import GenOneDataIDT


class DeePMO_IDT_PSRextinction(_DeePMO):
    """
    使用 Network_PlainSingleHead 网络结构进行 DeePMO 训练: 考虑 PSR 的具体数值，同时考虑当前 PSR 的熄火极限
    0. 在 init 中需要确定 init_res_time 参数，为了缩短计算时间
    1. 其他与 DeePMO_IDT_PSR_T.py 相同, 同样考虑分层采样范围，但是不考虑敏感度分析
    2. 在之外增加一个训练 A - extinction time 网络，能够使用所有的样本点（包括 PSR 点火失败）进行训练。该函数同样也需要加入
       网络筛选环节
    """
    def __init__(self, circ = 0, basic_set = True, GenASampleRange = True, setup_file: str = './settings/setup.yaml', 
                 cond_file: str = "./data/true_data/true_psr_data.npz", SetAdjustableReactions_mode:int = None, GenASampleRange_mode = None,
                 previous_best_chem:str = None, init_res_time = None, need_true_PSRex = False, 
                 PSRex_cutdown = None, IDT_cutdown = None, **kwargs) -> None:
        ...
        self.init_res_time = self.APART_args.get("init_res_time", self.RES_TIME_LIST[:,1]) if init_res_time is None else init_res_time
        # 确定真实情况下的点火极限，为 RES TIME LIST 中最后一个数值
        # 加载 PSRex 中每步的缩减指数 decay_exp: 计算 PSRex 时会以 0.5 ** decay_exp 为步长进行缩减
        self.PSRex_decay_rate = 2 ** self.APART_args.get("PSRex_decay_exp", 0.5)
        ...
        self.IDT_cutdown = self.APART_args.get("IDT_cutdown", False) if IDT_cutdown is None else IDT_cutdown
        ...
        self.WriteCurrentAPART_args(
            ...
            PSRex_decay_exp = self.APART_args.get("PSRex_decay_exp", 0.5),
    ...
    def GenASampleThreshold(self, best_chem_IDT, best_chem_PSR, best_chem_PSRex, threshold_expand_factor = None, **kwargs):
        ...
        if self.circ >= 1:
            ...
            if isinstance(psrex_threshold, Iterable):
                ...
                self.psrex_threshold = max([min(self.psrex_threshold, psrex_threshold), 1.5])
            else:
    ...
    def SortALIST(self, w1 = None, w2 = None, apart_data_path = None, experiment_time = 50, cluster_ratio = False, 
        ...
        if diff_psrex.max() < 1e-3:
            diff =  diff_idt + diff_psrex * w2[0] + diff_psr
        diff = diff_idt + diff_psrex * w2[0]
    ...
    def ASample(self, sample_size = None, coreAlist = None, IDT_weight = None, PSRex_weight = None,
        ...
        if self.circ == 0 or self.circ == start_circ:
            ...
            # true_psr = (self.true_psr_data - psr_mean) / psr_std
            # psr_threshold = psr_threshold / psr_std
            ...
            if coreAlist is None:
                cluster_weight = kwargs.get('cluster_weight', 0.1)
                previous_coreAlist = self.SortALIST(IDT_weight, PSRex_weight, 
                        apart_data_path = os.path.dirname(self.apart_data_path) + f'/apart_data_circ={self.circ - 1}.npz',
            ...
            while len(self.samples) < sample_size:
                ...
                for A0 in coreAlist:
                    try:  
                        tmp_sample, idt_pred_data = SampleAWithNet(net.forward_IDT, np.log10(self.true_idt_data), threshold = np.log10(self.idt_threshold), 
                        ...
                        if shrink_strategy:
                            tmp_boarder_sample = SampleAWithNet(net.forward_IDT, np.log10(self.true_idt_data), threshold = np.log10(self.idt_threshold) + 1, 
                    ...
                    if if_PSR_filter:
                        try:
                            tmp_sample, psr_pred_data = SampleAWithNet(net.forward_PSR, self.true_psr_data, threshold = self.psr_threshold, 
                                                        father_samples = tmp_sample, debug = True, scalar_input = psr_std, mean_input = psr_mean)
                            if shrink_strategy:
                                tmp_boarder_sample = SampleAWithNet(net.forward_PSR, self.true_psr_data, threshold = self.psr_threshold + 1, 
                                                        father_samples = tmp_boarder_sample, debug = False, scalar_input = psr_std, mean_input = psr_mean)                      
                    ...
                    if if_PSRex_filter:
                        try:  
                            tmp_sample, psrex_pred_data = SampleAWithNet(psrex_net, np.log2(self.true_extinction_time), threshold = np.log2(self.psrex_threshold), 
                                                        ...
                                                        reduced_data = np.log2(self.reduced_extinction_time))
                            if shrink_strategy:
            ...
            for index in range(sample_length):
                try:
                    future = exec.submit(
                            ...
                            PSR_exp_factor = PSRex_exp_factor,
                            **kwargs
    ...
    def _TrainDataProcess(self, load_file_name = None, concat_pre = False,  rm_tree = True, 
                              ...
                              PSRex_cutdown = None, IDT_cutdown = None, rate = 0.8, device = 'cuda', 
        ...
        # 如果样本点 PSRex 与真实值相差过大，我们不愿意见到这样的现象，故而删除这些样本点
        ...
        # 如果样本点 IDT 远超于简化值，我们不愿意见到这样的现象，故而删除这些样本点
        if IDT_cutdown:
            if_pass *= np.all(np.abs(all_idt_data - np.log10(self.true_idt_data)) <= np.log10(9), axis = 1)
        ...
        self.APART_args['input_dim'], self.APART_args['output_dim'] = passed_A.shape[1], [passed_idt_data.shape[1], passed_psr_data.shape[1], passed_T_data.shape[1]]
        ...
        self.TrainAnetLogger.info(f"The size of IDT & PSR train set is {self.APART_args['train_size']}; test set is {self.APART_args['test_size']}")
        ...
        dataset = DATASET_TripleHead(
            ...
            data_QoI2 = torch.tensor(passed_psr_data, dtype = torch.float32),
            data_QoI3 = torch.tensor(passed_T_data, dtype = torch.float32),
            device = device
        )
        ...
        for x_test, idt_test, psr_test, T_test in test_loader:
            self.test_loader = (x_test.to(device), idt_test.to(device), psr_test.to(device), T_test.to(device))
    ...
    def _DeePMO_train_T(self, device = None, division_ratio = 0, **kwargs):
        
        """
        在原有训练代码的基础上，增加了对 PSR_extinction 的训练。首先进行 IDT / PSR / T 的训练，
        紧接着再次单独训练一个 PSR extinction 网络
        """
        raise DeprecationWarning("This function is deprecated, please use _APART_train_T instead.")
        ...
        self.TrainAnetLogger.info(f'A -> IDT & PSR + T training has started...; circ = {self.circ}')
        ANET = Network_PlainDoubleHead_T(input_dim, self.APART_args['hidden_units'], output_dim, division_ratio = division_ratio).to(device)
        ...
        torch.save(state, f'{model_pth_path}/ANET_initialized_circ={self.circ}.pth')  # 保存DNN模型
        ...
        for epoch in range(self.APART_args['epoch']):
            # 预测
            with torch.no_grad():
                x_dnn_test, idt_test, psr_test, t_test = self.test_loader
                idt_pred, psr_pred, t_pred = ANET.forward_IDT(x_dnn_test), ANET.forward_PSR(x_dnn_test), ANET.forward_T(x_dnn_test)
                loss1 = criterion(idt_pred, idt_test); loss2 = criterion(psr_pred, psr_test); loss3 = criterion(t_pred, t_test)
                test_loss = [float(loss1.cpu().detach()), float(loss2.cpu().detach()), float(loss3.cpu().detach())]
            # 训练
            idt_loss, psr_loss, t_loss  = 0, 0, 0
            for _, (x_train_batch, idt_train_batch, psr_train_batch, t_train_batch) in enumerate(self.train_loader):  # 按照 batch 进行训练
                x_train_batch, idt_train_batch, psr_train_batch, t_train_batch = \
                    x_train_batch.to(device), idt_train_batch.to(device), psr_train_batch.to(device), t_train_batch.to(device)
                idt_pred, psr_pred, t_pred = ANET.forward_IDT(x_train_batch), ANET.forward_PSR(x_train_batch), ANET.forward_T(x_train_batch)
                loss1 = criterion(idt_pred, idt_train_batch); loss2 = criterion(psr_pred, psr_train_batch); loss3 = criterion(t_pred, t_train_batch)

                train_loss_batch = loss1 / loss1.detach() + loss2 / loss2.detach() + loss3 / loss3.detach()
                optimizer.zero_grad()
                train_loss_batch.backward()
                optimizer.step()
                idt_loss += float(loss1.detach()); psr_loss += float(loss2.detach()); t_loss += float(loss3.detach())
            
            scheduler.step()
            batch_num = len(self.train_loader)
            idt_loss /=  batch_num; psr_loss /=  batch_num; t_loss /=  batch_num
            train_his1.append([idt_loss, psr_loss, t_loss]); test_his1.append(test_loss)
            epoch_index.append(epoch);           

            if epoch % 5 == 0:
                GPUtil.showUtilization()
                self.TrainAnetLogger.info(f"epoch: {epoch}\t train loss: {idt_loss:.3e} + {psr_loss:.3e}  + {t_loss:.3e} "+
        ...
        self.best_pth_path = f'{model_pth_path}/early_stopping/model_best_stopat_{stop_index}_circ={self.circ}.pth'
        self.APART_args['best_ppth'] = self.best_pth_path
        try:
            shutil.copy(f'{model_pth_path}/tmp/ANET_epoch_{stop_index}.pth', self.best_pth_path)
        ...
        stop_index = int(earlystopping_step * np.argmin(test_loss_sum) + earlystopping_step / 2)
        self.best_pth_path = f'{early_stopping_file}/model_best_stopat_{stop_index}_circ={self.circ}.pth'
        self.APART_args['PSR_extinction_best_ppth'] = self.best_pth_path
        try:
            shutil.copy(f'{tmp_save_path}/ANET_epoch_{stop_index}.pth', self.best_pth_path)
            shutil.rmtree(tmp_save_path, ignore_errors=True); mkdirplus(tmp_save_path) # 删除中间文件 
        except:
            self.TrainAnetLogger.warning(f"copy file error, {tmp_save_path}/ANET_epoch_{stop_index}.pth not exist")
        ...
        return self.best_pth_path


    def _DeePMO_train(self, device = None, PSR_train_outside_weight = 1, **kwargs):
        ...
        for epoch in range(self.APART_args['epoch']):
            # 预测
            with torch.no_grad():
                x_dnn_test, idt_test, psr_test, _ = self.test_loader
                idt_pred, psr_pred = ANET.forward_IDT(x_dnn_test), ANET.forward_PSR(x_dnn_test)
                loss1 = criterion(idt_pred, idt_test); loss2 = criterion(psr_pred, psr_test)
                test_loss = [float(loss1.cpu().detach()), float(loss2.cpu().detach())]
            # 训练
            idt_loss, psr_loss = 0, 0
            for _, (x_train_batch, idt_train_batch, psr_train_batch, _) in enumerate(self.train_loader):  # 按照 batch 进行训练
                idt_pred, psr_pred = ANET.forward_IDT(x_train_batch), ANET.forward_PSR(x_train_batch)
                loss1 = criterion(idt_pred, idt_train_batch); loss2 = criterion(psr_pred, psr_train_batch)

                train_loss_batch = loss1 / loss1.detach() + loss2 / loss2.detach() * PSR_train_outside_weight
                optimizer.zero_grad()
                train_loss_batch.backward()
                optimizer.step()
                idt_loss += float(loss1.detach()); psr_loss += float(loss2.detach())
            
            scheduler.step()
            batch_num = len(self.train_loader)
            idt_loss /=  batch_num; psr_loss /=  batch_num
            train_his1.append([idt_loss, psr_loss]); test_his1.append(test_loss)
            epoch_index.append(epoch);           

            if epoch % 5 == 0:
                GPUtil.showUtilization()
                self.TrainAnetLogger.info(f"epoch: {epoch}\t train loss: {idt_loss:.3e} + {psr_loss:.3e} "+
    ...
    def DeePMO_train(self, concat_pre = False,  rm_tree = True, shrink_strategy = None, extract_strategy = None, 
        ...
        IDT_cutdown = self.IDT_cutdown if IDT_cutdown is None else IDT_cutdown
        ...
        ax[1].semilogy(train_his1[:,1], lw=1, label='PSRtrain')
        ax[1].semilogy(test_his1[:,1], 'r', lw=1.2, label='PSRtest')
    ...
    def SkipSolveInverse(self, w1 = None, w2 = None, father_sample:str = None, save_dirpath = f'./inverse_skip', 
        ...
        if not father_sample is None and os.path.exists(father_sample):
            ...
            inverse_alist = self.SortALIST(w1, w2, self.apart_data_path, experiment_time = experiment_time,
                    IDT_reduced_threshold = IDT_reduced_threshold, psr_mean = psr_mean, psr_std = psr_std)
        # 加载网络
        optim_net = load_best_dnn(Network_PlainDoubleHead, self.model_current_json, device = device)
        optim_psrex_net = load_best_dnn(Network_PlainSingleHead, self.model_current_json, device = device, 
                                        prefix = "PSR_extinction")
        for index in range(experiment_time):
            ...
            try:  
                ...
                final_idt = optim_net.forward_IDT(torch.tensor(A_init, dtype = torch.float32)).detach().numpy()
                ...
                # zscore scale
                final_psr = optim_net.forward_PSR(torch.tensor(A_init, dtype = torch.float32)).detach().numpy()
                final_psr = reverse_zscore(final_psr, psr_mean, psr_std, 
                ...
                self.InverseLogger.info("Compare First PSR:" + "\n" + f"True:{self.true_psr_data}; " + "\n" + f"Reduced:{self.reduced_psr_data};\n" + 
                                        f"Cantera:{cantera_psr_data};" + f"Final:{final_psr};")
                self.InverseLogger.info("-" * 90)                
    ...
    def SortALISTStat(self, apart_data_path = None, father_sample_path = None):
        ...
        previous_best_chem_IDT, _ = yaml2idt(
            ...
            IDT_condition = self.IDT_condition, fuel = self.IDT_fuel, oxidizer = self.oxidizer,
        ...
        previous_best_chem_PSR = yaml2psr(
            ...
            fuel = self.PSR_fuel, oxidizer = self.oxidizer, error_tol = 0.
        )   
        previous_best_chem_PSRex = yaml2psr_extinction_time(
            ...
            fuel = self.PSR_fuel, oxidizer = self.oxidizer, exp_factor = self.PSRex_decay_rate
        )
        ## 调用 sample_distribution 函数绘制 IDT 与 PSR 的分布情况
        ...
        IDT_func = lambda x: network.forward_IDT(torch.tensor(x, dtype = torch.float32)).detach().numpy()
        ...
        PSR_func = lambda x: reverse_zscore(network.forward_PSR(torch.tensor(x, dtype = torch.float32)).detach().numpy(), 
                                            psr_mean, psr_std, 
                                            replace_maskon_items = np.repeat(
                                            self.PSR_condition,
                                            repeats = 3,
    ...
    def IDT_SensitivityDesent(self, start_samples = None, target_IDT_conditions = None, iter_nums = 10, delta = 0.01,  
                            eve_sample_size = 50, max_step_size = 0.2, cpu_nums = None, father_sample_size = None,
                              result_analysis_dirpath = None, **kwargs):
        """
        ...
        benchmark_IDT, benchmark_T = yaml2idt(
        ...
        gatherAPARTdata(
            npz_keys = ['IDT'], apart_data_keys = ['idt'], save_path = "./data/SensitivityDesent",
            cpu_nums = cpu_nums, logger = logger, save_file_name= "./data/SensitivityDesentData.npz",
            **kwargs
        )
        ...
        diff_idt = np.linalg.norm(np.log10(all_idt_data) - np.log10(self.true_idt_data), ord = np.inf, axis = 1)
        ...
        print(Alist)
        if not result_analysis_dirpath is None:
            # 分析所有的结果
            mkdirplus(result_analysis_dirpath)
            for i, sample in enumerate(Alist):
                tmp_path = mkdirplus(dirpath + f"/{i}")
                print("="* 10)
                # 仿照 skip_inverse 中的内容进行分析
                Adict2yaml(self.reduced_mech, tmp_path + f"/sample.yaml", Alist = sample, eq_dict = self.eq_dict)
                # 计算 IDT 和 IDT 的敏感性
                IDT_sensitivity, base_IDT = yaml2idt_sensitivity(
                    tmp_path + f"/sample.yaml", delta = delta, 
                    IDT_condition = target_IDT_conditions, 
                    fuel = self.IDT_fuel, oxidizer = self.IDT_oxidizer,
                    IDT_mode = self.IDT_mode, cut_time = self.APART_args['idt_cut_time'],
                    need_baseidt = True
                )
                relative_error = np.mean(np.abs(base_IDT - benchmark_IDT) / benchmark_IDT) * 100
                compare_nn_train3(
                        np.log10(benchmark_IDT),
                        np.log10(base_IDT),
                        np.log10(self.reduced_idt_data),
                        ...
                        colors = ['blue', 'red'],
                        title = f'IDT  Relative Error: {relative_error:.2f} %',
                        save_path = tmp_path + '/compare_nn_IDT.png',
                        wc = self.IDT_condition
                    )
...
def GenOneDataIDTPSRextinction(IDT_condition: np.ndarray, PSR_condition: np.ndarray, RES_TIME_LIST: np.ndarray, Alist:list, 
                       eq_dict:dict, reduced_mech:str, index:int,  my_logger:Log, tmp_chem_path:str = None, fuel:str = None, oxidizer: str = None, 
                        IDT_mode = 0, remove_chem = True, idt_arrays:np.ndarray = 1, cut_time:np.ndarray = None, PSRex_decay_exp = 0.5,
    ...
    PSR_T, PSR_extinction = _GenOnePSR_with_extinction(PSR_condition, RES_TIME_LIST, PSR_fuel, PSR_oxidizer, tmp_chem_path, index, 
                                      my_logger, psr_error_tol = psr_error_tol, PSRex_decay_exp = PSRex_decay_exp, **kwargs)
    t1 = time.time()
