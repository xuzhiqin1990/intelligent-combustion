...
from APART_base import _GenOnePSR_with_extinction
from APART_plot.APART_plot import compare_nn_train3, sample_distribution_PSRex
...
class DeePMO_PSRextinction(_DeePMO):
        ...
        if GenASampleRange:
            if previous_best_chem is None:
                if self.circ == 0: 
                    ...
                    previous_best_chem, _, previous_best_chem_PSRex = self.SortALIST(
            ...
            self.GenASampleThreshold( 
                best_chem_PSRex = previous_best_chem_PSRex,
                **kwargs
            )
        else:
        ...
        if save_path is not None and os.path.exists(save_path):
            ...
            if need_true:
                ...
                self.true_extinction_time = yaml2psr_extinction_time(
                    self.detail_mech, self.PSR_condition, fuel = self.fuel, oxidizer = self.oxidizer,
            ...
            self.reduced_extinction_time = yaml2psr_extinction_time(
                self.reduced_mech, self.PSR_condition, fuel = self.fuel, oxidizer = self.oxidizer, 
                psr_tol = 10, init_res_time = self.init_res_time, exp_factor = self.PSRex_decay_rate
            )
    ...
    def GenASampleThreshold(self, best_chem_PSRex, threshold_expand_factor = None, **kwargs):
        """
        生成采样的阈值; 根据 best_chem_PSRex 生成采样阈值. 需要在 init 中就启动
        生成的具体策略为: 
        ...
        """
        self.GenAPARTDataLogger.info(f"Start GenASampleThreshold with best_chem_psr: best_chem_PSRex: {best_chem_PSRex}")
        ...
        self.GenAPARTDataLogger.info(f"Current CIRC Rlos are {psrex_Rlos}")
        if self.circ >= 1:
        ...
        self.GenAPARTDataLogger.info(f"Current CIRC thresholds are fixed {self.psrex_threshold}")
        self.GenAPARTDataLogger.info("=" * 50)


    def SortALIST(self, apart_data_path = None, experiment_time = 50, cluster_ratio = False, 
                PSR_reduced_threshold = None, return_all = False, father_sample_save_path = None,
                    logger = None, ord = np.inf, PSRex_cutdown = None,**kwargs) -> np.ndarray:
        """
        从 apart_data.npz 中筛选出来最接近真实 psr 的采样点，以反问题权重作为 psr 和 PSR 的筛选权重
        增加一个限制： 筛选出的结果到真实值的距离不能比 Reduced 结果差大于 psr_reduced_threshold 和 PSR_reduced_threshold 倍
        ...
        params:
            ...
            need_psr: 如果需要筛选出的 Alist 对应的 apart_data， 将此键值改为 True
        ...
        apart_data_psr = apart_data['all_psr_data']
        ...
        if experiment_time < 1: experiment_time = int(experiment_time * apart_data_psr.shape[0])

        assert Alist.shape[0] == apart_data_psr.shape[0] != 0, "Alist or apart_data_psr is empty!"
        ...
        # 初始化筛选向量 if_psr_pass
        if_psr_pass = np.ones(apart_data_psr.shape[0], dtype = bool)
        # 筛选出的结果不能比 Reduced 结果差大于  PSR_reduced_threshold 倍
        if not PSR_reduced_threshold is None:
            if_psr_pass *= np.all(np.abs(true_psr - apart_data_psr) <= PSR_reduced_threshold * np.abs(reduced_psr - true_psr), axis = 1)
            if if_psr_pass.sum() == 0: logger.warning(
        ...
        if PSRex_cutdown:
            ...
            if_psr_pass *= np.all(diff_psrex <= np.amin(diff_psrex, axis = 0) + 1, axis = 1)
            if if_psr_pass.sum() == 0: logger.warning(
        ...
        # 计算 DIFF_psr 和 DIFF_psr
        diff_psr = np.linalg.norm(true_psr - apart_data_psr, axis = 1, ord = 2)
        
        
        Alist = Alist[if_psr_pass, :]; apart_data_psr = apart_data_psr[if_psr_pass, :]
        diff_psr = diff_psr[if_psr_pass]; apart_data_psr = apart_data_psr[if_psr_pass, :]; apart_data_psrex = apart_data_psrex[if_psr_pass, :]
        diff_psrex = diff_psrex[if_psr_pass]
        # DIFF_psr 暂时删除或者只有 diff_psrex 非常小才启用
        if diff_psrex.max() < 1e-3:
            diff = diff_psrex  + diff_psr  / 1000
        diff = diff_psrex 
        index = np.argsort(diff); Alist = Alist[index,:]
        apart_data_psr = apart_data_psr[index,:]; apart_data_psrex = apart_data_psrex[index,:]
        ...
        if not father_sample_save_path is None:
           np.savez(father_sample_save_path, Alist = Alist[0:experiment_time, :], PSR = apart_data_psr[0:experiment_time, :]) 
        if return_all:
            return Alist[0:experiment_time, :] , apart_data_psr[0:experiment_time, :], apart_data_psrex[0:experiment_time, :]
        else:
            return Alist[0:experiment_time, :]
        

    def ASample(self, sample_size = None, coreAlist = None,
                passing_rate_upper_limit = None, if_PSR_filter:bool = True, if_PSRex_filter:bool = True,
                PSR_reduced_threshold = None, shrink_delta = None,
        ...
        """
        重写了 APART.ASample_psr_PSR 函数，主要是为了增加针对不同反应调节采样范围的函数
        ...
        # 预设置
        # 提取采样的左右界限 + 采样阈值
        # 检测类中是否存在 self.psr_threshold, self.psrex_threshold
        if not hasattr(self, 'psr_threshold') or not hasattr(self, 'psrex_threshold'):
        ...
        if self.circ == 0 or self.circ == start_circ:
            ...
            psrex_net = load_best_dnn(Network_PlainSingleHead, self.model_previous_json, device = 'cpu')
            ...
            self.GenAPARTDataLogger.info(f"psr_threshold: {np.amin(self.psr_threshold)} ~ {np.amax(self.psr_threshold)}; sample_size: {sample_size}; father sample size: {core_size}")
            ...
            if coreAlist is None:
                ...
                previous_coreAlist = self.SortALIST(
                        apart_data_path = os.path.dirname(self.apart_data_path) + f'/apart_data_circ={self.circ - 1}.npz',
                        experiment_time = core_size, psr_mean = psr_mean, psr_std = psr_std,
            ...
            t0 = time.time(); psrex_zero_father_sample_times = 0
            ...
            while len(self.samples) < sample_size:
                ...
                for A0 in coreAlist:
                    try:  
                        tmp_sample, psrex_pred_data = SampleAWithNet(psrex_net, np.log2(self.true_extinction_time), threshold = np.log2(self.psrex_threshold), 
                                                    size = tmp_sample_size,  A0 = A0, l_alpha = self.l_alpha,  r_alpha = self.r_alpha, save_path = None,  loss_func = psrex_loss_func, passing_rate_upper_limit = passing_rate_upper_limit, debug = True,
                        ...
                        if shrink_strategy:
                            tmp_boarder_sample = SampleAWithNet(psrex_net, np.log2(self.true_extinction_time), threshold = np.log2(self.psrex_threshold) + 1, 
                                                    ...
                                                    r_alpha = ((1 - shrink_delta) * self.r_alpha, (1 + shrink_delta) * self.r_alpha), 
                                                    reduced_data = np.log2(self.reduced_extinction_time), reduced_threshold = None, debug = False)
                        ...
                        self.GenAPARTDataLogger.info(f"PSRex: Sampled {len(tmp_sample)} in this iter, pass Rate is {len(tmp_sample) / tmp_sample_size * 100} %, total size is {len(self.samples) + len(tmp_sample)}! cost {time.time() - t0:.2f}s; Memory usage: {psutil.Process(os.getpid()).memory_info().rss:.2e} B")  
                        self.GenAPARTDataLogger.info(f"PSRex: Sampled {len(tmp_boarder_sample)} Boarder Data in this iter, total size is None! Memory usage: {psutil.Process(os.getpid()).memory_info().rss /1024 /1024 /1024:.2e} GB")  
                        self.GenAPARTDataLogger.info("——"*100)
                        if len(tmp_sample) == 0:
                    ...
                    # pass_rate 太高调小 psr_threshold 和 psrex_threshold
                    if pass_rate > passing_rate_upper_limit * 0.9 and pass_rate > 0.5:
                        self.psrex_threshold = self.psrex_threshold * 0.8
                        self.GenAPARTDataLogger.warning(f"PSRex: pass_rate is {pass_rate * 100} %, which is too high! psrex_threshold have changed to log2({self.psrex_threshold}) = {np.log2(self.psrex_threshold)}")
                    
                    # 样本太多立即退出
                    if len(self.samples) >= sample_size * 1.2:
                        self.GenAPARTDataLogger.warning(f"PSRex: Stop the Sampling Process, Total Size has come up to {len(self.samples)} data after this iter! cost {time.time() - t0:.2f}s")  
                ...
                if time.time() - t0 > sample_size * average_timeout_time:
                    ...
                    break
                self.GenAPARTDataLogger.info(f"In this Iteration, PSRex: Total Size has come up to {len(self.samples)} data after this iter! cost {time.time() - t0:.2f}s")  
                self.GenAPARTDataLogger.info(f"Memory usage: {psutil.Process(os.getpid()).memory_info().rss /1024 /1024 /1024} GB")
    ...
    def GenDataFuture(self, samples:np.ndarray = None,  psr_error_tol = None, 
    ...
    def _TrainDataProcess(self, load_file_name = None, concat_pre = False,  rm_tree = True, 
        ...
        if extract_strategy and self.circ != 0:
            ...
            for circ in range(self.circ):
                ...
                if os.path.exists(apart_data_path) and os.path.getsize(apart_data_path) /1024 /1024 > 1:
                    Alist, PSR, PSRex = self.SortALIST(
        ...
        all_psr_data, Alist_data, all_psr_extinction_data  = \
             np.array(all_psr_data), np.array(Alist_data), np.log2(all_psr_extinction_data)
        ...
        self.TrainAnetLogger.info(f"The size of all_psr_data is {all_psr_data.shape}, Alist_data is {Alist_data.shape}, all_psr_extinction_data is {all_psr_extinction_data.shape}")
        ...
        passed_T_data, passed_psr_data, passed_A = \
            all_psr_data[if_pass], Alist_data[if_pass]

        # 只保留 sample size 个数据点
        expected_sample_size = self.APART_args['sample_size'] if isinstance(self.APART_args['sample_size'], int) else self.APART_args['sample_size'][self.circ]
        if passed_psr_data.shape[0] > expected_sample_size:
            passed_psr_data, passed_A = \
                passed_psr_data[:expected_sample_size], passed_A[:expected_sample_size]

        assert passed_psr_data.shape[0] > 0 and passed_A.shape[0] > 0, \
            f"Error! passed_psr_data.shape = {passed_psr_data.shape}, passed_A.shape = {passed_A.shape}"
        ...
        self.APART_args['input_dim'], self.APART_args['output_dim'] = \
        ...
        self.TrainAnetLogger.info(f"The size of PSR extinction train set is {self.APART_args['train_size']}; test set is {self.APART_args['test_size']}")
        ...
        for x_test, psr_extinction_test in test_loader:
            self.test_loader = (x_test.to(device), psr_extinction_test.to(device))
    ...
    def _DeePMO_train(self, device = None, PSR_train_outside_weight = 1, **kwargs):
        ...
        self.TrainAnetLogger.info(f'A -> PSRex training has started...; circ = {self.circ}')
        tmp_save_path = mkdirplus(f'{model_pth_path}/tmp')
        input_dim, output_dim = self.APART_args['input_dim'], self.APART_args['output_dim']
        ...
        torch.save(state, f'{tmp_save_path}/Network_psr_epoch_{0}.pth')  # 保存DNN模型
        # 开始训练
        ...
        for epoch in range(self.APART_args['epoch']):
            # 预测
            with torch.no_grad():
                x_dnn_test, psr_extinction_test = self.test_loader
                psr_extinction_pred = ANET.forward(x_dnn_test)
                loss = criterion(psr_extinction_pred, psr_extinction_test)
                test_loss = loss.item()
            # 训练
            train_loss = 0
            for _, (x_train_batch, psr_extinction_train_batch) in enumerate(self.train_loader):  # 按照 batch 进行训练
            ...
            if (epoch == 0) or ((epoch - 25) % 50 == 0):
                ...
                torch.save(state, f'{tmp_save_path}/Network_psr_epoch_{epoch}.pth')  # 保存DNN模型  
        ...
        try:
            shutil.copy(f'{tmp_save_path}/Network_psr_epoch_{stop_index}.pth', best_pth_path)
            shutil.rmtree(tmp_save_path, ignore_errors=True); mkdirplus(tmp_save_path) # 删除中间文件 
        except:
            self.TrainAnetLogger.warning(f"copy file error, {tmp_save_path}/Network_psr_epoch_{stop_index}.pth not exist")

        # 保存实验结果
        np.savez(f'{model_loss_path}/Network_psr_circ={self.circ}.npz', # 保存DNN的loss
    ...
    def DeePMO_train(self, concat_pre = False,  rm_tree = True, shrink_strategy = None, extract_strategy = None, 
        ...
        self.TrainAnetLogger.info(f"ANET_train_PSR: Device is {device}")
        ...
        tmp_data = np.load(f'{self.model_loss_path}/Network_circ={self.circ}.npz')
        train_his2 = tmp_data['train_his']; test_his2 = tmp_data['test_his']; stop_index2 = tmp_data['stop_index']
        
        fig, ax = plt.subplots(1, 1, figsize = (16, 4))
        ax.semilogy(train_his2, lw=1, label='PSRextinction_train')
        ax.semilogy(test_his2, 'r', lw=1.2, label='PSRextinction_PSRtest')
    ...
    def SkipSolveInverse(self, father_sample:str = None, save_dirpath = f'./inverse_skip', 
                             csv_path = None, device = 'cpu', raw_data = False, experiment_time = 15, **kwargs):
        """
        ...
        for index in range(experiment_time):
            ...
            try:  
                ...
                cantera_psr_data = yaml2psr(inverse_path + '/optim_chem.yaml', PSR_condition = self.PSR_condition, RES_TIME_LIST = self.RES_TIME_LIST,
                                               fuel = self.fuel, oxidizer = self.oxidizer, error_tol = 0.)
                ...
                cantera_psrex_data = yaml2psr_extinction_time(inverse_path + '/optim_chem.yaml', PSR_condition = self.PSR_condition, RES_TIME_LIST = self.RES_TIME_LIST,
                                                  fuel = self.fuel, oxidizer = self.oxidizer, exp_factor = self.PSRex_decay_rate)
    ...
    def gather_apart_data(self, save_path = "./data/APART_data/tmp", save_file_name = None,  rm_tree = True, 
            ...
            if cover or not os.path.exists(save_file_name) or os.path.getsize(save_file_name) /1024/1024 <= 1:
                ...
                return all_psr_data, Alist_data, all_psr_extinction_data
            else:
    ...
    def gather_apart_data_PSRco(self, save_path = "./data/APART_data/tmp", save_file_name = None,  rm_tree = True, 
            ...
            if cover or not os.path.exists(save_file_name) or os.path.getsize(save_file_name) /1024/1024 <= 1:
                ...
                if rm_tree:
                    ...
                    mkdirplus("./data/APART_data/blank_dir")
                    subprocess.run(f"rsync --delete-before -d -a ./data/APART_data/blank_dir/ {save_path}/", shell = True)
                return all_psr_data, Alist_data, all_psr_extinction_data, all_psr_concentration_data
            else:
                self.GenAPARTDataLogger.warning("gather_apart_data function is out-of-commision")
    ...
    def SortALISTStat(self, apart_data_path = None, father_sample_path = None):
        ...
        # 读取数据
        apart_data = np.load(apart_data_path)
        Alist = apart_data['Alist']; PSR = apart_data['all_psr_data']
        ...
        # 调用 sample_distribution 函数绘制 psr 与 PSR 的分布情况
        ## 加载上一步中的最优样本点
    ...
    def PSRex_SensitivityDesent(self, start_samples = None, target_PSR_conditions = None, iter_nums = 10, delta = 0.01,  
        ...
        Returns:
            Alist: 二维数组，最终的样本点
             all_psr_data, all_psr_extinction_data: 与 Alist 对应的 idt, PSR, PSRex 数据
        """
        ...
        data = np.load("./data/SensitivityDesentData.npz"); Alist = data['Alist']
        ...
        return Alist, all_psr_data, all_psr_extinction_data


@func_set_timeout(300)
def GenOneDataPSRex(PSR_condition: np.ndarray, RES_TIME_LIST: np.ndarray, Alist:list, 
                       eq_dict:dict, reduced_mech:str, index:int,  my_logger:Log, fuel:str = None, oxidizer: str = None,  tmp_chem_path:str = None, 
                        remove_chem = True, PSR_EXP_FACTOR = 0.5, psr_error_tol = 50, init_res_time:np.ndarray = 1, save_path = 'data/APART_data/tmp', 
                        PSR_fuel:str = None, PSR_oxidizer: str = None, **kwargs):
    """
    将 IDT 和 PSR 结合起来
    ...
    PSR_condition = np.array(PSR_condition); RES_TIME_LIST = np.array(RES_TIME_LIST)
    ...
    if isinstance(PSR_T, int):
        return PSR_T
    else:
        my_logger.info(f"Mechanism {index} : cost {time.time()-t0:.2f} s, the first PSR element is {PSR_T[0:3]}, the first PSR extinction time is {PSR_extinction[0:3]}, " + 
                       f"Memory usage: {psutil.Process(os.getpid()).memory_info().rss /1024 /1024 /1024:.2e} GB")
        np.savez(save_path, PSR = PSR_T, Alist = Alist, PSR_extinction = PSR_extinction)
        return None
